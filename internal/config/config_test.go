...
// through atomicfile.WriteFile, so it must not turn a symlinked pickle.toml
...
// atomicfile.WriteFile, so it must report the same actionable, real-path-first
...
// D5: a failure inside atomicfile.WriteFile must be reported against the
...
func TestSetPayloadVersionInPlaceUnwritableParentNamesTheRealFile(t *testing.T) {
	...
	msg := err.Error()
	// atomicfile.WriteFile resolves symlinks in its own path before reporting
...
// (Save on a path that does not exist yet), not just atomicfile.WriteFile in
// isolation.
