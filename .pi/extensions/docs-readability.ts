 ...
 * `.agents/docs-readability.prompt.md`.
 *
 * This is DEV tooling for building pickle — it is NOT part of the ticket flow
 * pickle ships (the embedded `skill/` payload). Hence the shared prompt lives
 * outside `skill/`, so it is never bundled into `pickle install`.
 *
...
// Kept OUT of `skill/` so it is never embedded into the shipped `pickle install`.
const PROMPT_PATH = ".agents/docs-readability.prompt.md";
