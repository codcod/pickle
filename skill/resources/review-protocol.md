...
- **Child-project (build target):** present the full Conventional Commit message —
  ...
  **publishing**. Only after the user approves all attributes: finalize the branch (squash to
  ...
  and **create the merge request** in that child-project's repo. Publishing follows the
...
- [ ] Summary + child-project commit message & MR attributes presented for approval (commit/push/MR per the project's commit policy; merge stays human) + overarching-repo bookkeeping committed per policy + next-ticket suggestion (step 9)
