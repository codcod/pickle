...
- **Child-project (build target):** present the full Conventional Commit message —
  ...
  **Before pushing, verify the remote base is not behind your local base**: `git fetch origin
  ...
  push `origin <base>` first and re-check. Only
  then push and **create the merge request** in that child-project's repo. Publishing follows the
...
- [ ] Summary + child-project commit message & MR attributes presented for approval (commit/push/MR per the project's commit policy; merge stays human), **remote base verified not behind local (`origin/<base>...HEAD` carries no `tickets/` path, after a fetch) before pushing**, + overarching-repo bookkeeping committed per policy + next-ticket suggestion (step 9)
