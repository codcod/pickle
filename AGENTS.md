...
- **The foreign-workspace test** (T-098). Everything under `skill/` is read by projects that are
  ...
  payload, ask: *would this help a project that is not pickle?* Three shapes fail it — a **ticket
  ...
  ("13 header variants across the corpus" — whose?), and **"this repo" meaning ours** (the
  `field-use`/`self-host` split was once defined as "another project" versus "this repo's own
  flow", which no foreign team can assign). Two uses of a ticket id stay legitimate, because
  ...
  sentence stand on its own, or to phrase it relative to the skill the reader is holding
