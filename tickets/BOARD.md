...
|---|---|---|---|---|---|---|
| T-128 | doctor: warn when a feature branch's ticket file is stale relative to the base branch | medium | medium | M | [] |  |
...
| T-076 | consume rick status --json as an artifact-state source | yes — merged to main (PR #84, 8147a63) |
| T-077 | show a ticket's rick artifacts in pickle serve, read-only, with the effective-instance rule | yes — merged to main (c09a241, PR #85) |
...
| T-102 | surgical ticket frontmatter field writer: single-field edit behind a parse-back guard | yes — merged to main (a4bb5d0, PR #83); all 7 commits kept as history rather than squashed (the root-path default, rule… |
