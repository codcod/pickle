...
**Verdict: REWORK, then fixed — scoped re-review pending.** Two blocking findings, both one
theme: the redefinition landed in the code and in the files the plan enumerated, but the
*sweep* for the semantics it abolished stopped there. The implementation itself was sound —
all five tasks met, all seven confirmed decisions honoured, acceptance test green, self-host
protection intact. Both blocking findings (F1, F2) were fixed on the same branch (commit
`6a47764`), scope limited to exactly those two — see the findings table for what changed.
Re-run: `TestSkillDirLabel*`, `TestUpgradeAlwaysReplacesSkillDirWholesale`,
`TestUpgradeSurvivesAnUnreadableSkillEntry`, `TestMarkerBlockGolden`,
`TestSelfHostMarkerBlockIsCurrent` all pass; `just build && just test && just lint &&
just docs-check` clean. No behaviour changed — comments and docs only.
...
- [x] Rework: F1 and F2 fixed on the same branch (commit `6a47764`), scope limited to the two
      listed findings; acceptance test and build/test/lint/docs re-run green (rework procedure
      step 4)
- [ ] Summary + commit message presented for approval (step 9) — deferred: pending the scoped
      re-review this hand-back requests.
