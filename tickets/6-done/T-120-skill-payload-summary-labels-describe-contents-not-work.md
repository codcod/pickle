...
**Verdict: DONE** (round 1 REWORK → scoped re-review passed).

Round 1 raised two blocking findings, both one theme: the redefinition landed in the code and in
the files the plan enumerated, but the *sweep* for the semantics it abolished stopped there. The
implementation itself was sound — all five tasks met, all seven confirmed decisions honoured,
acceptance test green, self-host protection intact. Both blocking findings were fixed on the same
branch, scope limited to exactly those two.

**Scoped re-review (round 2) — both resolved.** Independence: **delegated** again (the reviewing
agent authored the rework fix in this session), to a fresh sub-agent briefed adversarially and
asked specifically whether the fix was superficial, incomplete, or regressive. Its findings were
re-verified by hand before recording — including the two it judged "legitimately scoped", which
the reviewer confirmed independently rather than accepting (`cli-reference.adoc:379` sits inside
`[#cmd-upgrade]` describing that command's own behaviour, and its marker bullet already credits
`install`; `install.go:427` contrasts the skill dir with `pickle.toml`, not with `install`).

- **F1 resolved.** All three cited locations corrected, and an independent whole-tree sweep
  (`docs/`, `README.md`, `DESIGN.md`, `CHANGELOG.md`, `skill/`, `internal/` incl. CLI help text)
  found no surviving claim that only `upgrade` replaces or prunes the skill dir, none that the
  *payload* is "refreshed in place", and none that `(current)` means file contents. The new
  sentence's factual claim was checked against the code, not just read: `copyPayload` is the
  single wholesale path, while `installPi` writes each scaffold individually with no `RemoveAll`
  and `ensureSymlink` re-points one link — so "refreshed individually instead" is accurate.
- **F2 resolved.** `labelSkillDir` and the package doc now carry decision-1 wording, consistent
  with `skillPayloadDiffers` and the manual's table.
- **Marker block, three copies verified in agreement** (`install.go`, this repo's `AGENTS.md`,
  `testdata/markerblock.golden`), machine-checked by `TestSelfHostMarkerBlockIsCurrent`; the
  golden diff contains only the intended sentence.
- **Scope respected.** Filtering the rework's `install.go` hunk for lines that are neither
  comments nor string literals returns zero — no logic, test or behaviour change.
- **Cross-file xref proven, not assumed.** The reviewer broke `<<skill-dir-labels>>` in a throwaway
  copy and confirmed `TestDocsXrefsResolve` fails on it (`snowball check` alone passes blindly) —
  so the anchor is genuinely gated, not merely present.

Re-run on the tidied branch: acceptance test verbatim (unit set + the throwaway `pickle-test`
hand check, which reproduced `+ … (refreshed)` → stale dir gone → `= … (current)` on both
commands → `doctor` 0/0), plus `just build && just test && just lint && just docs-check` — all
clean. The tidy was verified content-preserving: the tidied tree differs from the reviewed tree
in `tickets/` paths only.
...
- [x] Rework: F1 and F2 fixed on the same branch, scope limited to the two listed findings;
      acceptance test and build/test/lint/docs re-run green (rework procedure step 4)
- [x] Scoped re-review (round 2): F1 and F2 verified resolved by an independent delegate whose
      findings were re-verified by hand; two new non-blocking prose findings (F11, F12) fixed
      inline; branch tidied into two atomic commits, verified content-preserving
- [x] Summary + child-project commit messages presented for approval (step 9). Publishing gated
      on that approval; **`main`'s bookkeeping commits must be pushed before the branch**, so
      the in-tree `origin/main...HEAD` check comes back clean.
...
|---|---|---|---|---|---|---|
| F1 | blocking | docs-gap | — | User-facing docs still attribute wholesale replacement to `upgrade` alone, or call the skill dir refreshed "in place" — contradicting decision 3 and, in one case, the new section on the same page. `install` now *deletes* anything the payload does not ship, and `project-structure.adoc` is the page a user reads to learn what is safe to keep where. | `docs/user-manual/concepts/project-structure.adoc:47-49` ("`pickle upgrade` replaces these wholesale"); `docs/user-manual/cli-reference.adoc:207-209` ("refreshed in place", five lines above the section stating it wipes and re-copies every run); `cli-reference.adoc:216` ("The one-line summary `install` prints", although `upgrade` xrefs this block as the definition of *its* label) | **Fixed in `6a47764`.** `project-structure.adoc`'s pickle-owned bullet now names both commands for `{skill-dir}/` and separates it from the `.claude` symlink/`.pi` scaffolds (still refreshed individually); `cli-reference.adoc`'s "refreshed in place" is scoped to the marker block, and the skill-dir-labels section now says "`install` and `upgrade` print". The marker block shipped into every installed project's `AGENTS.md` (`install.go:1374-1375`) was updated the same way and hand-mirrored into this repo's own `AGENTS.md` + `testdata/markerblock.golden` per the self-modify policy — this closes the impact-sweep note below about `T-121` too, since the sentence it plans to leave untouched is the one just corrected. |
| F2 | blocking | stale-xref | — | `labelSkillDir` — the one function that *emits* the label — still documents the definition this ticket abolished, verbatim contradicting **T-120 decision 1**. The package doc carries the same abolished "refreshed in place" framing. | `internal/install/install.go:185-186`: "`"(current)"` means the bytes on disk already matched"; `internal/install/install.go:5-6` | **Fixed in `6a47764`.** `labelSkillDir`'s comment and the package doc both now say the skill payload is replaced wholesale (decision 1's "changed nothing observable on disk" wording); the marker block stays scoped to what it actually does (refreshed in place). |
...
**Round 2 findings (scoped re-review).** Both raised against prose *this branch authored*, so both
take the inline disposition per rules §5 ("prose or idiom this branch authored, or made false —
and no behaviour change"):

| id | severity | class | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|---|
| F11 | non-blocking | docs-gap | fixed inline | The F1 fix narrowed "replaces these wholesale" to `{skill-dir}/` and in doing so stopped saying that the `pickle:begin…end` region is regenerated wholesale too — a completeness gap the fix itself introduced. Stated correctly in `cli-reference.adoc`, so a reader of that page alone was unaffected. | `docs/user-manual/concepts/project-structure.adoc:48-52` | Fixed: the bullet now says both commands replace the skill dir **and** re-inject the marker region, while the `.claude` symlink and `.pi` scaffolds are refreshed individually. |
| F12 | non-blocking | other | fixed inline | "install **no longer** just adds to it" is release-relative phrasing on a timeless reference page — a first-time reader has no prior behaviour to contrast against. | `docs/user-manual/concepts/project-structure.adoc:49-50` | Fixed: rephrased to state what install does ("pruning anything the payload does not ship"), with no appeal to what it used to do. |

**Dispositions (both rounds):** 2 blocking (F1, F2 — fixed in rework, re-verified resolved) ·
5 fixed inline (F3, F4, F5, F11, F12) · 5 noted (F6, F7, F8, F9, F10) · 0 folded · 0 new tickets —
no finding passed the promotion test; F6–F10 stay recorded here with their evidence for a later
reviewer to promote by citing the row.

cost: estimated M, actual M — the code landed at estimate; the two review rounds were spent
almost entirely on the documentation sweep the plan under-scoped to three named files.
