...
Reviewed 2026-08-12 on `feat/T-046-self-host-aware-doctor` (3 commits at review start:
`dd4b06c`, `bbcec7d`, `c691320`). Audited by the reviewing agent plus an **independent
adversarial sub-agent** briefed to find defects, which mutation-tested the branch — that is
what produced F3, the one finding with real teeth.

- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a)
- [x] Docs-readability pass on the changed `.adoc`/`.md` files (step 4b) — ran; 13 suggestions,
      1 applied (F7), 12 declined as out-of-scope churn on prose this branch never touched
...
**Acceptance test — re-run verbatim, all green.** `just build`, `just test` (forced
`-count=1`, not the cached run), `just lint`, `just docs-check` all clean. The five named
tests pass, none skipped. On this self-hosting repo: `./pickle doctor` → `0 error(s), 0
warning(s)`, exit `0` (was `0 error(s), 1 warning(s)`); `doctor -v | grep -i 'payload
version'` → exactly one `ok:` line, no `WARNING:`, no `pickle upgrade`; the
`pre-commit guard installed and current` regression guard still prints `1`. Negative case in a
throwaway dir: an ordinary install with a drifted stamp still warns and still names
`pickle upgrade`, and its skill line is the generic one — so the skip is scoped to the link,
not to doctor. `board audit`: 97 tickets, 0 errors, 0 warnings.

**Design decisions D1–D8: all honoured.** Verified individually, including D5's ordering (the
skip is `checkVersion`'s first statement) and D6 (no other check suppressed — `checkHooks`,
`checkMarkers` and `checkAgentScaffolds` are untouched; only `Check`, `checkSkill` and
`checkVersion` changed).

### Findings

| id | severity | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|
| F1 | non-blocking | fixed inline | `SkillLinked`'s doc comment claimed install, upgrade **and uninstall** all treat the link as "not ours to replace". `Uninstall` in fact *removes the link itself* (`os.Remove`), refusing only to `RemoveAll` through it. The comment also referenced `checkSkill`, an unexported func in another package. | `internal/install/install.go:58-67` (new comment) vs `:406-415` (`Uninstall`'s actual symlink branch) | Rewritten: names what each command really does, drops the cross-package reference. |
| F2 | non-blocking | fixed inline | **D5 was only half-pinned.** A mutant moving the skip below the `version==""\|"dev"` guard *and* the equal-version early return (still above the `PayloadVersionStampable` probe) passed the **entire** suite. Only the probe-ordering half was guarded. The code was always correct; the contract was not defended. | Reproduced the mutation and re-ran `-run SelfHost`: green. After the fix it fails with the intended message; restored and re-verified. | `TestCheckSelfHostLinkNamesTheLink` already checks at version == stamp, so one `hasPassedContaining(…, "payload version check skipped")` there kills the mutant. Added. |
| F3 | non-blocking | fixed inline | `checkSkill`'s self-host branch recomputed `filepath.Join(root, filepath.FromSlash(install.SkillDir))` for `Readlink`, a path already in scope as `dir`. | `internal/doctor/doctor.go:102` | Use `dir`; comment why `Readlink` needs the un-followed path. |
| F4 | non-blocking | fixed inline | `cli-reference.adoc` stated flatly that `payload_version` "**is** still stamped" over a self-host link. `Upgrade` returns early without writing when already at the target version, so the claim is stronger than the code. | `internal/install/install.go:355-358` (`if cfg.PayloadVersion == payloadVersion { … return }`) | Qualified with "whenever it actually differs (an already-current version is a no-op)". D4's substance is unchanged. |
| F5 | non-blocking | fixed inline | CHANGELOG entry was missing a verb: "(`…/ticket-flow` a symlink to the payload source)". Also the one docs-readability suggestion that landed on prose this branch authored. | `CHANGELOG.md` `[Unreleased]` → `Changed` | Insert "is". |
| F6 | non-blocking | noted | Routing `Upgrade` through `!SkillLinked(root)` is a genuine semantic widening: the old guard required `Lstat` to *succeed*, the new one also takes the destructive branch when `Lstat` errors. Benign in practice — `RemoveAll` on a missing path returns nil, and a non-ENOENT error (e.g. EACCES) failed the upgrade before too, just one step later with a different message. | `internal/install/install.go:301` vs the pre-branch guard | No code change. The asymmetry (`false` = "not a link" *or* "cannot tell") is now spelled out in the predicate's doc comment, since `!SkillLinked` guards a `RemoveAll`. |
| F7 | non-blocking | noted | The `target = "?"` fallback in `checkSkill` is effectively unreachable: `selfHost` came from an `Lstat` on the same path in the same process, so only a TOCTOU race reaches it, and nothing exercises it. | `internal/doctor/doctor.go:103-108` | Kept — correct defensive handling of a real (if vanishing) race. Now documented as such rather than looking like an untested branch. |
| F8 | non-blocking | folded → **T-042** | `copyPayload`'s guard is `err == nil && fi.Mode()&os.ModeSymlink != 0` on the same path, with `fi` used for nothing else — i.e. *exactly* `SkillLinked(root)`. The plan excluded it on the rationale that it "needs `Lstat`'s existence answer as well as the mode", which is true of `Uninstall` but **false of `copyPayload`**. The code correctly followed the decision; the decision's stated reason was wrong. | `internal/install/install.go:541` | Added as T-042 item 4 (that epic owns predicate collapsing). No behaviour change available here anyway. |
| F9 | non-blocking | folded → **T-042** | The self-host symlink fixture is built twice (`selfHostFixture` in `doctor_test.go`; inline in `hooks_test.go`). The duplication is *justified* — the two need different bases (`installFixture` vs `gitFixture`) and cannot compose — but a 4-line `linkSkill(t, root)` would serve both. | `internal/doctor/doctor_test.go:38-58`, `internal/doctor/hooks_test.go:182-192` | Added to T-042 item 4, alongside its existing test-helper unification (item 3 touches these same two files). |
| F10 | non-blocking | noted | The independent audit flagged `TestCheckSelfHostLinkSkipsVersionCheck`'s loop over `Passed` as near-vacuous. **Reviewer disagrees after checking D3**, which requires exactly this: "the string `pickle upgrade` must not appear in the line". The loop asserts over `Passed`, which the warnings check does not cover. | plan D3 vs `internal/doctor/doctor_test.go:346-352` | No change. Recorded so a later reader does not "simplify" a deliberate assertion away. |

**Disposition summary:** 10 findings, **0 blocking** — 5 `fixed inline` (F1–F5, all in this
branch's own prose/idiom/tests, no behaviour change), 2 `folded` into **T-042** (F8, F9),
3 `noted` (F6, F7, F10). **No new tickets**: nothing here passes the promotion test — the two
substantive leftovers land squarely on an existing epic that already owns predicate and
test-helper collapsing.

The inline fixes are commit `79ab742` on the same branch; the acceptance test and all four
configured commands were re-run green afterwards, and the F2 mutation re-verified as caught.
