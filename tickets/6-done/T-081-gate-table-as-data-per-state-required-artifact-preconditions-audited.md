...
### Scoped re-review (2026-08-11)

Re-reviewed on `feat/T-081-gate-table` (7 commits, `d744867`..`cb0cf98`), ticket read from `main`.
Scope per the protocol: verify B1, B2, B3 and the folded N4 — not a re-audit of the feature.
Verdict: **all three blocking findings resolved → `6-done/`**; two new non-blocking findings in
the rework's own prose, both **fixed inline**, and two noted.

- [x] Blocking findings re-verified (B1, B2, B3) — each reproduced live in a throwaway install
- [x] Acceptance test re-run verbatim (all four steps) + `just build`/`test`/`lint`/`docs-check`
- [x] Consistency sweep over the rework diff only (the two rework commits)
- [x] Docs-readability pass on the rework's changed `.adoc` files (step 4b) — 14 suggestions, 13
  against prose this ticket never touched (out of scope for its diff); the one on branch-authored
  text is the `lifecycle.adoc` READY-gate paragraph, which R2 reopened and corrected
- [x] Findings recorded with severity **and** disposition; disposition summary present (step 5)
- [x] Ticket moved to `6-done/`; `## History` appended (step 6b)
- [x] Impact sweep concluded — T-085's coupling note patched (the deferral from the first review)
- [ ] Summary + commit messages & MR attributes presented for approval (step 9)

**B1 — resolved.** `gateRemedy` derives the second way out from `def.Allowed(dir)`. Enumerated
over every state in `flow.Default()`: only `2-ready` yields a move-naming remedy, and it names
`1-to-do` — legal from READY. `3-in-development`, `4-in-review` and `5-rework` all fall back to
`— write it to satisfy the gate`. Verified live: a `4-in-review` ticket missing
`### Acceptance test` now gets the generic remedy, where before it was told to run a move
`ticket move` refuses (`illegal transition IN REVIEW → TO DO`, reproduced for contrast).
`TestGateRemedyOnlyNamesLegalNonBlockingTargets` is a genuine property test over the real
definition, and the `TestAudit` case pins the in-review regression by message text.

**B2 — resolved.** `lifecycle.adoc`'s false clause is gone. Verified live at the strongest form
of the claim: a `2-ready` ticket with its **entire** `## Implementation Plan` section deleted
still moves to `1-to-do` (exit 0) and leaves `board audit` at 0 errors. The escape hatch is open
and now correctly scoped to `2-ready/`.

**B3 — resolved (coverage present and accurate after R1).** All three documented failure modes
reproduced live with one legacy violating ticket in the tree: `post-upgrade audit found 1
error(s)` exit 1, `post-install audit found 1 error(s)` exit 1, and an unrelated
`ticket move T-002 dropped` → `move applied but board audit now reports 1 error(s)` exit 1 with
the file already in `7-dropped/`. The manual's parenthetical that the install/upgrade itself
still completes is correct (`internal/cli/install.go:91,180` run the self-check after the write).

| id | severity | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|
| R1 | non-blocking | fixed inline | The B3 fix introduced a false comparison: the manual said the gate table "is the only one [of the four upgrade-visible shapes] that can make **`pickle upgrade` and `pickle install` themselves fail**, not only `board audit`". All four shapes are `board audit` **errors**, and the one post-operation self-check aborts on any of them — so the sentence tells a reader the other three migrations are safe to upgrade through. | Throwaway install: deleting a ticket's `spawned-by:` key (shape 1) → `pickle: post-upgrade audit found 1 error(s)` exit 1 and `post-install audit found 1 error(s)` exit 1; deleting `tickets/7-dropped/` (shape 3) → `post-upgrade audit found 2 error(s)` exit 1. `docs/user-manual/cli-reference.adoc` gate-table paragraph of the `cmd-board-audit` NOTE; the same claim in `CHANGELOG.md` ("the one migration note in this release"). | Fixed inline (rules §5 — prose this branch authored and made false, no behaviour change): both places now say every one of the four fails `upgrade`/`install` alike, and name the real difference — the other three clear with one `board sync`, one `install` re-run or one backfilled `spawned-by: []` line, while this one needs a heading written by hand per affected ticket. Commit `cb0cf98`. |
| R2 | non-blocking | fixed inline | B2's replacement sentence overshot: `lifecycle.adoc` called `1-to-do/` "the one status the plan rows never apply to". `6-done/` and `7-dropped/` declare no requirements either (decision 4), and `lifecycle.adoc` says so nowhere — so the page implies an archived ticket with no plan is flagged. | `docs/user-manual/concepts/lifecycle.adoc`, READY-gate paragraph, against `brine.go`'s terminal states and `cli-reference.adoc` ("`6-done/` and `7-dropped/` require nothing"). The docs-readability reviewer independently rephrased the same sentence, inheriting the same imprecision. | Fixed inline, same commit: scoped to "the one *non-terminal* status", with the archive exemption stated in a following parenthesis. |
| R3 | non-blocking | noted | `gateRemedy` picks the **first** legal non-terminal target with no Blocking row and phrases it as "move the ticket **back** to X". Direction is not checked — it holds only because `1-to-do` happens to be the sole such target today (from `2-ready`, `3-in-development` is tried first and skipped for having its own Blocking rows). A flow whose forward state carried no Blocking row would render "move the ticket back to 3-in-development", and the property test would still pass: it asserts legality and non-blockingness, not direction. | `internal/audit/audit.go` `gateRemedy`; enumerated remedies for all seven states (only `2-ready` names a move). | Either drop "back" from the wording or assert the target's ordinal is lower than `dir`'s. Noted, not scheduled: project-authored flows are explicitly out of scope (decision 10), so no second flow can reach this today, and the doc comment already explains the derivation. |
| R4 | non-blocking | noted | `gateRemedy(def, t.Dir)` is re-derived inside the per-violation loop, so a ticket missing its whole plan derives the identical string 8 times (each walking `Allowed` × that state's `Requires`). | `internal/audit/audit.go`, the `r.errf` call in the gate-table loop. | Hoist it above the loop, computed once per ticket. Noted: at brine's scale (90 tickets) it is unmeasurable, and the current shape keeps the call adjacent to the message it decorates. |

**Disposition summary (re-review):** 4 findings — 0 blocking, 4 non-blocking: 2 fixed inline (R1,
R2 — both prose this branch authored and made false), 2 noted (R3, R4), 0 folded, 0 new tickets.
The three original blocking findings (B1, B2, B3) are all verified resolved. Cumulative across
both passes: 11 findings, 3 blocking (all fixed), 8 non-blocking (5 noted, 2 fixed inline, 1
folded into T-042), **0 new tickets spawned**.

**Impact sweep concluded (step 8).** The deferred T-085 patch is applied: its coupling note
recorded T-081's unit as *"a `##` section (and its non-emptiness)"*; as shipped the unit is a
`##` section **plus** a normalised `###` stem inside it, and the rework did not move it, so the
note is now correct rather than provisional. Per T-085's own standing instruction, no re-grade.
No `1-to-do/`/`2-ready/` ticket declares `depends-on: [T-081]`; T-065's only mention of T-081 is
a historical grading precedent, unaffected.
