...
D2=$(mktemp -d) && cp "$REPO/pickle" "$D2/pk" && cd "$D2" && git init -q
printf '// mine\n{}\n' > opencode.jsonc
OUT=$(./pk install --project demo --agent opencode)   # no pipe: grep -q would SIGPIPE pk under pipefail
grep -qi 'opencode.jsonc' <<<"$OUT"
...
D3=$(mktemp -d) && cp "$REPO/pickle" "$D3/pk" && cd "$D3" && git init -q && ./pk install --project demo
...
**2026-07-26 — full review** (protocol + no configured addenda; verdict: done)

- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage verified; no docs build configured (step 4a)
- [x] Docs-readability pass run via the pi `docs_readability` tool on README.md, .pi/README.md,
      .opencode/README.md, skill/resources/review-protocol.md (step 4b) — suggestions
      presented to the user; nearly all target prose outside this ticket's diff
...
- [x] Impact sweep: T-013 item 4 and T-019 items 1–2 patched — their "--agent is a no-op"
      assumptions were falsified by this ticket (step 8)
- [x] Summary + commit message & MR attributes presented for approval (step 9)

**Implementation audit.** All 9 tasks verified in the tree: prompt shipped at
`skill/resources/docs-readability.prompt.md` (git-mv, dev tooling repointed); embedded
`agents/` tree + `//go:embed all:skill all:agents`; `--agent` parsing with usage error +
`--no-claude` deprecation shim (`internal/cli/install.go`, `install.ParseAgents`);
install/upgrade/uninstall symmetry incl. opencode absent/present policy and `.pi` pruning
(`internal/install/install.go`); doctor drift warning (`internal/doctor/doctor.go`);
Step 4b + checklist line in the shipped protocol (SKILL.md/tickets-README sweep found no
step-number references needing changes; pickle.toml has no addenda); tests
(`internal/{install,cli,doctor}/agents_test.go`); README (`--agent` contract, per-agent
table, reviewer, opencode policy). All 9 confirmed decisions honoured — verified the shipped
guardrails carry rules 1+2 only (the dev self-modify guard does not ship), the shipped/dev
pi extension diff is comment-only, and `git log --follow` shows the prompt moved unmodified.
`just build` / `just test` / `just lint` clean; `pickle board audit` clean.

**Findings table**

| id | severity | disposition | description | evidence | suggestion/resolution |
|---|---|---|---|---|---|
| F1 | non-blocking | fixed inline | Acceptance script step 1 fails verbatim: throwaway dirs are never `git init`-ed, so `./pk doctor` errors (`child "demo": . is not a git repository`) and `set -e` aborts. Product behaviour is correct; harness defect this branch authored. | verbatim run exited 1 at `./pk doctor` | `git init -q` added to all three throwaway dirs in the ticket's script; re-run green (`ACCEPTANCE OK`, doctor 0/0) |
| F2 | non-blocking | fixed inline | Acceptance script step 2 dies with SIGPIPE (exit 141) under `set -o pipefail`: `./pk install \| grep -qi` — grep exits at first match while pk is still printing the long manual-merge note. | verbatim run exited 141 at step 2 | pipeline replaced with capture-then-grep in the ticket's script; re-run green |
| F3 | non-blocking | fixed inline | `skill/SKILL.md`'s "Bundled resources" list omits the newly shipped `resources/docs-readability.prompt.md` — this branch made the list incomplete. | skill/SKILL.md:23–28 vs skill/resources/ | bullet added (prose only, no behaviour) |
| F4 | non-blocking | folded | `pickle help` says install works "for detected agents" — contradicts the no-autodetection contract (decision 1). Pre-existing wording (predates this branch), now more misleading. | `internal/cli/cli.go` usage text | folded into T-013 (install polish), new item added there |

**Disposition summary:** 4 non-blocking, 0 blocking — 3 fixed inline (F1, F2, F3), 1 folded
into T-013 (F4), 0 noted, 0 new tickets.

**Docs-readability suggestions (step 4b, not findings):** ~20 suggestions returned; all but
three target prose outside this ticket's diff (long-standing README/protocol phrasing).
Presented to the user with a recommendation to discard for this review.
