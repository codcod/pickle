...
2026-08-22 — review on `feat/T-066-close-cli-surface-doc-gaps`, per the review protocol. No
layered addenda configured (`pickle.toml` has neither an overarching nor a per-child
`review_addendum`). `depends-on: []`, so no dependency-merge gate.

- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a)
- [x] Docs-readability pass (step 4b) — ran `docs_readability` on the two `.adoc` files this
  ticket touched. Every suggestion on pre-existing paragraphs (the locking-model intro, hooks,
  changelog-check, upgrade bullets, the keys' closing paragraph) targets prose *outside* this
  ticket's diff — not applied, same precedent as T-048/T-110. The one suggestion that did land
  on new text — splitting the `--family`/`--spawned-by` sentence in `<<cmd-ticket-new>>` — was
  applied (`b1ca52a`).
...
- [x] Remaining-tickets impact sweep done (step 8) — **T-067** cites T-066 as a soft coupling
  only ("no file overlap"); re-checked, still true, no patch needed
- [x] Summary + commit message & MR attributes presented for approval (step 9)

**Implementation audit** (step 2) — all 5 tasks met, verified on the actual tree (branch
`feat/T-066-close-cli-surface-doc-gaps`, `db8ad25` + the review fix-inline commit `b1ca52a`):

- Task 1 (`--family` on `<<cmd-ticket-new>>`) — met: synopsis and prose both updated
  (`cli-reference.adoc:670,691-695`).
- Task 2 (`project add` flags) — met: all seven flags now listed in `<<cmd-project>>`'s bullet
  (`:238-241`).
- Task 3 (`pickle flow show|list`) — met: new `[#cmd-flow]` section (`:270-284`), Overview-table
  row (`:37-38`), and `configuration.adoc`'s `flow` key bullet all present; anchor is unique and
  correctly placed between `[#cmd-project]` and `[#cmd-upgrade]`.
- Task 4 (reword `renumber`) — met: both `skill/resources/tickets-README.md:266-269` and
  `internal/audit/audit.go:101-105`'s comment now state plainly that no such command ships;
  the payload wording is foreign-workspace-safe (no ticket id to look up, no first-person
  "this repo").
- Task 5 (glob markup bug) — met: `.pi/extensions/+*+.ts` passthrough at `cli-reference.adoc:391`
  (line moved down by the new `[#cmd-flow]` section) — **re-rendered end-to-end** with `asciidoctor`
  and `snowball build`'s actual PDF (`pdftotext` on the built output): the glob survives intact
  and `<strong>` no longer opens across it.
- Acceptance test — **re-run verbatim, all seven checks green**: all four `rg` greps confirm
  the documented content; `just docs-check` clean; the glob fix confirmed by direct render
  (both `asciidoctor` and a full `just docs-build` → `pdftotext` on the actual shipped PDF);
  `just build && just test && just lint` all clean.
- Confirmed design decisions 1–5 — all honoured: no flag/manual diff test was added (decision
  1); new flags landed beside their siblings, no new flag sections (decision 2); `[#cmd-flow]`
  sits between `cmd-project`/`cmd-upgrade` matching `pickle help`'s ordering, and documents
  `"brine"` as the one legal constant (decision 3); both `renumber` references were reworded,
  not replaced with new promises (decision 4); the PDF fix used a passthrough, not a backslash
  escape (decision 5).

**Quality/consistency audits** (steps 3–4) — docs-only + one code-comment change; nothing to
flag on idiom or correctness. Whole-tree sweep for the same glob-in-backtick bug
(`` `[^`]*\*[^`]*` `` across `docs/*.adoc` and `docs/user-manual/*.adoc`) found only two other
matches, both false positives (adjacent, unrelated backtick spans) — confirms the plan's claim
that line 368 (pre-`cmd-flow`) was the only real instance. `pickle help`'s own `usage()` text
for `project add` does not list flags either way (pre-existing terseness, out of this ticket's
stated scope: it compares the CLI against the *manual*, not against `pickle help`) — not a
finding.

**Documentation audit** (step 4a) — coverage: all five gaps closed in the files that already
cover their respective commands, no new page. Whole-tree sweep (above) found one gap: no
`CHANGELOG.md` entry (F1, below). `just docs-check` and a full `just docs-build` (PDF + EPUB)
...
|---|---|---|---|---|---|---|
| F1 | non-blocking | docs-gap | fixed inline | No `CHANGELOG.md` `[Unreleased]` entry for a payload-text fix (`tickets-README.md`) and a rendered-PDF/EPUB markup fix — both user-visible outside this repo, same category T-098 set precedent for entering the changelog | `pickle changelog check` flagged T-066 as an unnamed candidate before the fix | Added a `### Fixed` entry under `[Unreleased]` (`b1ca52a`); `pickle changelog check` now reports no candidates |

**Disposition summary:** 1 finding — 1 fixed inline (F1); 0 blocking, 0 noted, 0 folded, 0 new
tickets.

`cost: estimated S, actual S`

**Verdict:** no blocking findings → DONE. The one fixed-inline finding is already on the
branch (`b1ca52a`, alongside the docs-readability polish noted above); `just build && just
test && just lint && just docs-check` and `pickle changelog check` all re-verified clean after
the fix commit.
