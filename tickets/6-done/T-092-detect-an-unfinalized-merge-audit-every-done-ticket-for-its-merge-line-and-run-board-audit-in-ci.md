...
2026-08-13 — reviewed on `feat/T-092-unfinalized-merge-audit` (3 commits, `74cb8d2`/`340c608`/
`13a0519`). **Verdict: DONE — no blocking findings.**

- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a)
- [x] Docs-readability pass — **skipped**: the configured Gemini backend errored
      (`model_not_supported`), a sanctioned conscious skip (protocol step 4b)
...
(re-run after the review's inline fixes).

**Behaviour verified live**, in a throwaway install (`mktemp -d` + copied binary, per the
self-modify policy): the new warning fires exactly once on a `6-done/` ticket with no merge
line at **exit 0**; appending `- 2026-08-13 — merged to main (abc1234)` clears it to
`0 error(s), 0 warning(s)`; and reproducing the T-081 `duplicate id` **error** class exits
**1** — so the new CI step can genuinely fail, which task 4 demanded. The repo's own board
still audits `93 tickets, 0 error(s), 0 warning(s)`. All six confirmed design decisions were
honoured, including decision 5 (state derived from `def.DependencySatisfied()`, never a
hard-coded `"6-done"`) and decision 3 (a dedicated test asserts **exactly two** warnings on
**different refs** when a done ticket is also a `depends-on` target).

| id | severity | disposition | finding | evidence | suggestion |
|---|---|---|---|---|---|
| N1 | non-blocking | fixed inline | `DESIGN.md` §7 claimed wiring `board audit` into CI "remains worthwhile and **unshipped**" — this branch shipped exactly that | `DESIGN.md:244` vs `.github/workflows/ci.yml` | rewritten to record T-092 as the ticket that shipped it; the docs-gate half stays flagged unshipped (T-067) |
| N2 | non-blocking | fixed inline | Two live docs asserted an archived ticket "is never flagged" — true of the gate table, falsified at page level by the first check that *does* flag a `6-done/` ticket | `docs/user-manual/cli-reference.adoc:574`, `docs/user-manual/concepts/lifecycle.adoc:56` | both scoped to the gate table and pointed at the new warning; one clause added to lifecycle's *Done ≠ merged*, the concept's natural home |
| N3 | non-blocking | fixed inline | `skill/SKILL.md`'s *audit the board* paragraph enumerates every invariant, omitted the new one, and carried the same falsified "never flagged" sentence | `skill/SKILL.md:288–295` | user ruled (review, option 1) that decision 6 bars new *instruction* prose, not a correction plus one clause in a **tooling enumeration**; both applied |
| N4 | non-blocking | noted | The plan's acceptance test is **not runnable verbatim**: `ticket move T-001 done` is illegal from `1-to-do` (`legal: DROPPED, READY`), and the intervening `ready` move is refused by the READY gate on a fresh scaffold | live run: `pickle: illegal transition TO DO → DONE`; then `cannot move T-001 to READY: 8 unmet gate requirement(s)` | authored at refinement, not by this branch, so below the inline bar (causation, not size). The working fixture route is hand-move + a `TO DO → DONE` History line + `board sync`; every expected result then reproduced exactly |
| N5 | non-blocking | noted | The new check adds a **fourth** full walk over `tickets`, directly beneath a loop whose own comment says it was "folded into this loop so the tickets are not walked a third time" | `internal/audit/audit.go:194` vs `:253` | fold into the History↔directory loop if that file is next touched; irrelevant at 93 tickets, so not worth scheduling alone |
| N6 | non-blocking | noted | CI runs `board audit`, which only *warns* on layout-only board drift, while `board sync --dry-run` — which the manual itself recommends wiring into CI — exits non-zero on it. Layout-only staleness therefore stays uncaught | `cli-reference.adoc:674`, `concepts/project-structure.adoc:112` | add a `board sync --dry-run` step too, if layout drift ever actually bites |
| N7 | non-blocking | noted | Fifth upgrade-visible shape: any project whose `6-done/` predates the merge-line convention now emits one warning per ticket. Nothing fails (warnings never gate), but the manual's `[NOTE]` block enumerating the four upgrade-visible shapes does not mention it | `cli-reference.adoc` `[NOTE]` block, ~585–635 | that block is explicitly about *errors* failing install/upgrade, so a warning does not belong in it unqualified; revisit only if the noise is reported |
| N8 | non-blocking | noted | The message hardcodes `DONE`/`MERGED` rather than `def.DependencySatisfied().Name`, in a package that otherwise derives everything from the flow definition | `internal/audit/audit.go:262` | identical to the existing neighbour at `:239`, so no regression; a flow-genericity sweep would fix both at once or neither |

**Disposition summary:** 8 non-blocking findings, 0 blocking — 3 fixed inline (N1, N2, N3),
5 noted (N4, N5, N6, N7, N8), 0 folded, 0 new tickets.

**Impact sweep (step 8).** `T-091` is the only non-terminal ticket whose text this change
falsifies: its soft-coupling note read "running `board audit` in CI, **which nothing does
today**". Patched in place, per the sweep's expected outcome. `T-093` needs nothing — its
refinement had already decoupled it (decision 3 reads commit subjects, not merge History lines).
