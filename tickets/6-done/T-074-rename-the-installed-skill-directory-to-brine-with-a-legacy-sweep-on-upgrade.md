...
- [x] Ticket moved to `5-rework/`, then — after the scoped re-review below — to `6-done/`;
...
### Scoped re-review — F1 verified, ticket passes

Re-reviewed 2026-08-13 on `feat/T-074-rename-skill-dir-to-brine` (ticket read from the base
branch). Scoped to F1 per the protocol — the feature was not re-audited from scratch; F2–F7 keep
the dispositions recorded above.

**F1 — resolved.** The fix is `MarkerTitle = "Brine"` declared beside `MarkerBegin`/`MarkerEnd`,
with all four `injectMarker` call sites routed through it. Verified three ways:

1. **Diff scope.** `git diff 1c3a11e..HEAD` touches only `internal/install/install.go` (+15/-4),
   `internal/install/install_test.go`, and F2's one-line comment fix — no unrelated work rode in
   on the rework pass.
2. **Mutation-tested, because a regression guard that cannot fail is not a guard.** Three
   mutations, each reverted after: a single call site restored to the `"Ticket flow"` literal (the
   original defect) → **FAIL**; `MarkerTitle` itself reverted to `"Ticket flow"` → **FAIL**;
   `MarkerTitle` set to an unrelated third name (`"Pickle"`) → **initially PASS**, which is F8
   below.
3. **Live, on both code paths.** A fresh `install` into an empty repo now writes `# Brine` as the
   H1 of `AGENTS.md` *and* `CLAUDE.md`, with zero occurrences of "Ticket flow" in either; deleting
   both files and running `upgrade` (the `RefreshMarkers` path, which has its own two call sites)
   also re-creates them headed `# Brine`.

**Acceptance test re-run in full** (all 8 steps, `pickle-test` in a throwaway dir, using the
amended step 2 grep): green, including the legacy sweep steps 5–7 — `doctor` exit 1 → `upgrade`
reports both legacy paths removed → `doctor` exit 0 with the H1 preserved; the self-host legacy
link re-linked to `../../payload` with its tree and sentinel intact; `uninstall -n` listing both
legacy paths without mutating, then removing them. `just build && just test && just lint &&
just docs-check` all clean. `board audit`: 99 tickets, 0 errors, 0 warnings.

#### One new finding, from the re-review itself

| id | severity | class | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|---|
| F8 | non-blocking | test-gap | fixed inline | F1's own regression assertion built its expected value from `MarkerTitle` — the same constant the production code writes — so it moved with any edit to that constant and could not fail on its own. The paired `Contains("Ticket flow")` assertion did have teeth, so F1 was genuinely caught; but the H1 line itself pinned nothing | Mutation 3 above: `MarkerTitle = "Pickle"` left `TestRunProducesInstall` **green** | Fixed in `658395b`: the expected H1 is spelled out as the literal `"# Brine\n"`. All three mutations now fail. A test that restates the implementation constant is not asserting anything — worth spelling out even when the constant is right |

Disposition summary (re-review): 0 blocking, 1 non-blocking — 1 fixed inline (F8 in `658395b`).
Cumulative across both passes: 8 findings — 1 blocking (F1, fixed in rework `a25a53e`), 3 fixed
inline (F2, F7, F8), 4 noted (F3–F6). No follow-up ticket minted at either pass.

cost: estimated M, actual M — unchanged by the rework; F1 was a five-line fix, and the rework
pass cost less than the review that found it.
