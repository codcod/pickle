...
- [x] Reviewer independence settled (step 0): **delegated** — the orchestrating reviewer authored
  ...
  adversarially. Every delegated finding was re-verified by hand before recording; all seven
  held (see the evidence column — each was re-checked against the actual source, not accepted
  on the sub-agent's word).
- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2): all four
  tasks present in the files the plan names; all five confirmed design decisions honoured (no
  `skill/` or Go changes in the diff, no future verb named, no roadmap item). Required xref
  counts met: `<<agent-session-workflow>>` ×1 (plan says once), `<<cmd-changelog-check>>` ×5
  (plan says at least twice). `just build`, `just docs-check`, `just lint`, `just test` all PASS;
  `just docs-build` renders and the page is not an orphan (`TestDocsUserManualHasNoOrphanPages`).
- [x] Quality audit (step 3): the page's factual claims about `changelog check` (advisory,
  read-only, never blocks, one direction only) match `#cmd-changelog-check`. Its non-zero-exit
  claim was verified empirically against `internal/cli/changelog.go` — a *missing* changelog
  exits 1 by the same path as an unreadable one, so the claim is true but was imprecisely worded
  (F5).
- [x] Consistency audit (step 4): whole-tree sweep found no other page duplicating this one.
  Three cross-references were wrong or unsupported (F1–F3) and one pre-existing ambiguity was
  created by this branch (F4); all four fixed. All cited anchors resolve.
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a).
- [x] Docs-readability pass (step 4b): run on `releasing.adoc` and `installation.adoc`. Applied
  8 suggestions on prose this ticket authored; discarded 3 — two on pre-existing
  `installation.adoc` paragraphs this ticket never touched, and one rewording of the opening
  line ("not a phrase {product} understands" is deliberate: the page is about a trigger phrase).
  Readability polish only; none entered the findings table.
...
- [x] Remaining-tickets impact sweep done (step 8): T-113 and T-114 both patched — see below.
...
|---|---|---|---|---|---|---|
| F1 | non-blocking | correctness | fixed inline | "Its examples show why" pointed at examples that do not exist — `#cmd-changelog-check` has exactly one code block, the usage synopsis; the tagged-`--until` fact is prose. | `releasing.adoc:39`; only `----` pair in `cli-reference.adoc:1161-1293` is the synopsis | Cite the command's advisory `note:` line instead. Done. |
| F2 | non-blocking | correctness | fixed inline | The page attributed a "no-inference discipline" to `<<agent-session-workflow>>`; that page contains no infer/guess language and claims register parity only. | `releasing.adoc:8-10`; `grep -i 'infer\|guess' agent-session-workflow.adoc` → no matches | State the no-inference rule as this page's own; move the asw xref to the register claim in "What this is not". Done — xref count still exactly 1. |
| F3 | non-blocking | stale-xref | fixed inline | `<<lifecycle>>` was cited as the home of the publish gate, but `lifecycle.adoc` never mentions push, publish or approval. | `releasing.adoc:47`; `grep -i 'publish\|push\|approval' lifecycle.adoc` → no matches | Cite `<<your-first-project>>`, where publish-gating is actually explained (`:94`). Done. |
| F4 | non-blocking | docs-gap | fixed inline | This branch gave the manual a second, unsignposted sense of `RELEASING.md`: `installation.adoc` used it for pickle's own maintainer file, the new page for the reader's. | `installation.adoc:30,51` vs `releasing.adoc` | One disambiguating clause at both `installation.adoc` sites pointing at `<<releasing>>`. Done. |
| F5 | non-blocking | spec-unclear | fixed inline | The "`CHANGELOG.md` missing" bullet justified itself with "*unreadable*", leaving the missing case formally unstated. Behaviour is correct (missing also exits 1). | `releasing.adoc:62-63`; `internal/cli/changelog.go` unconditional `os.ReadFile` → `errf` | Reword to "a changelog it cannot read, whether it is absent or malformed". Done. |
| F6 | non-blocking | other | fixed inline | The new table cell answered "what these files are" rather than the column's contract, "Why it is tracked". | `project-structure.adoc:108-111`; column header `:75` | Rephrase as a tracking reason. Done. |
| F7 | non-blocking | other | fixed inline | New prose had one 117-char line against the file's ~85-char wrap, and stated "`pickle install` writes neither" twice within 24 lines. | `releasing.adoc:7`, `:6` and `:27-28` | Rewrap; drop the restatement. Done — longest line now 84. |

**Disposition summary:** 7 findings, all non-blocking — 7 fixed inline (F1–F7), 0 folded, 0 new
ticket, 0 noted. Every one is prose this branch authored or made ambiguous, with no behaviour
change, which is the `fixed inline` bar in rules §5; each is recorded above rather than silently
edited. No blocking findings.
