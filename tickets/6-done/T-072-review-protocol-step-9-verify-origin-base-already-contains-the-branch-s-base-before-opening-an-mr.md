...
**Scoped re-review (F1 only), after rework**

F1 fixed at both cited sites (`TEMPLATE.md:50`, `:109`→`:112`): both now carry "verify the base
 is not behind" + the same `git diff --name-only origin/<base>...HEAD | grep '^tickets/'` check
+ the repair, cross-referencing rules §0 rather than restating its rationale. Wording agrees in
substance with the §0 and step-9 formulations (verified by grep across all three files); no new
inconsistency introduced. `just build && test && lint && docs-check` and `pickle board audit`:
