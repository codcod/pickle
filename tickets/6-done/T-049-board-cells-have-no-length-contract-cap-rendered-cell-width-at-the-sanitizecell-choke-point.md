...
2026-07-27 — review of `feat/T-049-board-cell-width-cap` (commits `47bad16`, `0848d36`).
...
- [x] Docs-readability pass on the changed `.adoc`/`.md` files (step 4b) — run via the pi
      `docs_readability` tool on `docs/user-manual/cli-reference.adoc` + `CHANGELOG.md`; it had no
      suggestions for the CHANGELOG and none for the passages this ticket changed (its six
      cli-reference suggestions all target pre-existing prose elsewhere in the file) — discarded as
...
|---|---|---|
| Task 1 — cap in `sanitizeCell`, `maxCellRunes` constant beside `cellBreakRE`, doc comment updated | **met** | `internal/board/board.go:119-152`; constant at `:128` documents the legibility bound, D3, non-configurability and the 117-rune corpus max |
| Decisions 1-6 (one cap at the choke point; 120 inclusive of `…`; runes not bytes; cap last; head-preserving; no `depends-on` exemption) | **met** | `sanitizeCell` caps after trim via `[]rune`; `renderRow` (`:186-192`) is the only caller, so all nine columns inherit it; no per-column branch added in `cellFor` |
| Task 2 — `TestRenderCapsCellWidth` with the planned cases | **met** (5 subtests, cases 5+6 merged — see F4) | `go test ./internal/board/ -run TestRender -v` → 8 tests / 5 subtests PASS |
| Existing renderer tests untouched and green | **met** | `git diff main...HEAD --numstat`: `board_test.go` = 141 insertions, **0 deletions**; golden, purity, sanitisation and derived-cell tests all PASS |
| Task 3 — docs | **met** | `docs/user-manual/cli-reference.adoc:375-381` (derived-board list) extended; `:262` left as-is (not an exhaustive list — the plan's judgement call); `your-first-project.adoc:107` checked, says the cell "renders from that line" and does not claim verbatim, so correctly untouched |
| Task 4 — CHANGELOG with the migration note | **met** | `CHANGELOG.md:11-25`, including the `board audit` staleness ripple (decision 7) |
| Acceptance: `just build && just test && just lint && just docs-check` | **met** | all green (re-run after this review's inline fixes) |
| Acceptance: this repo's board unchanged, audit clean | **met** | `./pickle board sync` → `already in sync`; `diff` empty; `board audit: 52 tickets, 0 error(s), 0 warning(s)`; widest cell measured at **117 runes**, exactly as decision 2 claims |
| Acceptance: cap verified end-to-end on a throwaway workspace | **met** | throwaway dir + copied binary (AGENTS.md self-modify policy): a done ticket with a 900-rune merge line rendered a **120-rune** cell ending in `…`, `board audit` clean |
| Acceptance: decision 7's ripple confirmed deliberately | **met** | board re-rendered with a `main`-built binary (936-rune cell), then the new binary: `ERROR: BOARD.md is stale or hand-edited — run pickle board sync`, exit 1 — the documented behaviour |
| Docs build | **met** | `just docs-check` (snowball) clean |
| Commit convention | **met** | `fix(board): cap rendered cell width at 120 runes (T-049)` — id in brackets at the end of the subject |

### Findings (step 5)

| id | severity | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|
| F1 | non-blocking | **fixed inline** | `TestRenderCapsCellWidth`'s doc comment claimed the test verifies that the cap "runs after the ¦ substitution" — the exact claim amended decision 4 retracts, and it contradicted the comment inside its own case 4 ("this does *not* pin the two steps' order"). Prose this branch authored and made false. | `internal/board/board_test.go:183-184` vs `:240-244` | Reworded to what the case verifies (composition + row integrity), naming the amendment. Applied. |
| F2 | non-blocking | **fixed inline** | The upgrade ripple was documented only under `board sync`, but the error it produces is emitted by `board audit` — whose section already carries the precedent "*Upgrading an existing project*" NOTE (for `spawned-by:`) and enumerates the staleness error without mentioning the new cause. Discoverability gap created by this branch; coverage itself was present, so non-blocking. | `docs/user-manual/cli-reference.adoc` `[#cmd-board-audit]` NOTE block vs `:375-381` | Added a second paragraph to that NOTE cross-referencing `<<cmd-board-sync>>`. Applied. |
| F3 | non-blocking | **noted** | Cosmetic: when the truncation boundary falls on a space, the cell renders a space before the ellipsis (`…aaaa …`). The 120-rune invariant still holds. | probe: `sanitizeCell(strings.Repeat("a",118)+" "+strings.Repeat("b",50))` → tail `"aaaa …"`, 120 runes | A `TrimRight` before appending `…` would fix it, but that is a behaviour change, so it fails the inline bar and does not pass the promotion test either. |
| F4 | non-blocking | **noted** | Task 2 / the acceptance test ask for "six cases"; five subtests shipped, because planned cases 5 and 6 are asserted together in `merged cell keeps prefix and ref` (prefix, ref, width, `HasMergeLine`, and the `no — publish-gated` negative). Substantively complete — letter-of-the-plan drift only, and the test comment says so (`// 5/6 — …`). | `internal/board/board_test.go:238-289`; `go test -run TestRender -v` output | None. Merging them is the better test; the plan's count was the estimate. |
| F5 | non-blocking | **noted** | The cap is asserted through two columns (`title`, `merged`); `depends-on`, `reason` and `open findings` inherit it structurally because `renderRow` is the single caller of `sanitizeCell`. No per-column assertion exists. | `internal/board/board.go:186-192` | Leave it: per-column cases would assert that a `for` loop iterates, not a policy. The choke-point design is what makes them redundant. |
| F6 | non-blocking | **noted** | The new test helper `cellsIn` addresses cells by magic index (`[1]` title, `[2]` merged) instead of deriving the offset from `SectionColumns`, so a future column insertion shifts the assertions silently — the same fragility T-045 records for the old row mechanics. | `internal/board/board_test.go:176-201`, used at `:206`, `:270` | If a column is ever added, resolve the index via `SectionColumns(status)`; not worth a change now (the helper is 20 lines and its callers are three). |
| F7 | non-blocking | **noted** | Line-number references to `internal/board/board.go` in other tickets are stale and this branch shifted them further (`sanitizeCell` moved `:124` → `:142`): T-040 cites `board.go:29` for `rowRE` (actually `:50`, stale since T-044) and T-045 cites `board.go:120`/`:236`/`:282-288` for pre-T-044 mechanics that no longer exist. | `tickets/1-to-do/T-040-…:43`, `tickets/1-to-do/T-045-…:99-105` | Pre-existing (T-044 caused it), so it fails the inline bar. Both passages already flag themselves as pre-T-044; refinement of either ticket will re-read the file anyway. |
| F8 | non-blocking | **fixed inline** | Impact sweep (step 8): T-043's item-5 deferral enumerates the settled sanitisation as "`|` → `¦`, newlines → space" — an enumeration this branch made incomplete. | `tickets/1-to-do/T-043-…:57-58` | Enumeration completed with the 120-rune cap + a dated History line on T-043; item 5's scope is unchanged. Applied. |
| F9 | non-blocking | **noted** | The branch carries an unrelated bookkeeping commit (`a3ded99`, filing T-051/T-052 + `NOTES.md` from the second-child onboarding triage), so the diff under review is not minimal: 3 of 9 changed files are not T-049's. Permitted by the commit policy (overarching bookkeeping, explicit pathspecs) and unavoidable while pickle self-hosts in the same repo as its child. | `git log --oneline main..HEAD` | Consequence for step 9: **do not squash** — squashing would put two unrelated tickets under the T-049 subject. Keep the history, or cherry-pick the bookkeeping commit to `main` separately. |
| F10 | non-blocking | **noted** | Decision 8's accepted gap, re-confirmed: the `### <child>` sub-headings and the WIP-limit preamble render `p.Name` from `pickle.toml` without passing through `sanitizeCell`, so they are neither pipe-safe nor capped. | `internal/board/board.go:225-247` | Correct call for this ticket (a pathological child name is a config problem). If it ever matters, it is a `config` validation finding, not a renderer one. |

**Disposition summary:** 10 findings, **0 blocking** — 3 *fixed inline* (F1, F2, F8), 7 *noted*
(F3, F4, F5, F6, F7, F9, F10), 0 *folded*, 0 *new ticket*. No follow-up ticket passed the
promotion test; the docs-readability pass produced no in-scope suggestions.
