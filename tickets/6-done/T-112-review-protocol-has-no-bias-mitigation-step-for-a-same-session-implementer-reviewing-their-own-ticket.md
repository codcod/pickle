...
- [x] Reviewer independence settled (step 0): **delegated** — the reviewing agent authored the
      branch in this same session, so the audits went to two independently spawned reviewers
      (implementation+quality; consistency+docs), each briefed adversarially and told not to defer
      to the ticket's framing. Every finding below was re-verified by hand before being recorded;
      one auditor framing was downgraded on that re-verification (see F2). This ticket's own review
      is the first application of the step it ships.
- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a)
- [x] Docs-readability pass on the changed `.adoc`/`.md` files (step 4b) — 11 suggestions returned;
      2 touched lines this ticket changed and are folded into F4/F6's inline fixes, 9 targeted
      pre-existing prose outside this diff and were discarded as out of scope.
...
**Acceptance test re-run verbatim:** `just build`, `just test` (incl.
`TestPayloadSpeaksToAForeignReader`), `just lint`, `just docs-check` — all clean, before and after
the inline fixes. All four of the ticket's specific checks pass: no step renumbered (`## 0.` then
`1.`–`9.` with `4a`/`4b` intact); the payload's `T-[0-9]`/`this repo`/`the corpus` matches are
byte-identical in content to `main`'s; a throwaway `pickle-test install --in-tree` carries step 0
and the checklist line (`grep -c` = 2) plus the §8 pointer; docs build clean.

**All 6 tasks met, all 8 confirmed decisions honoured.** No blocking findings from either auditor.

| id | severity | class | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|---|
| F1 | non-blocking | spec-unclear | fixed inline | §8's new cross-reference was inserted mid-paragraph, so the following sentence's "The scope is not only assumptions that **aged**" read as the *review's* scope, not the pickup gate's; "the same principle" also over-promised, the pickup gate being unconditional where step 0 fires only on same-session authorship | `skill/resources/tickets-README.md:534-536` as shipped | moved the sentence to the paragraph's end and hedged to "the same bias …, on a narrower trigger" |
| F2 | non-blocking | spec-unclear | fixed inline | Step 0's prose mentioned recording only inside the degradation clause, so the always-on requirement was carried solely by the checklist line. Both auditors read this as under-serving decision 2; on hand-verification it is an **understatement, not a contradiction** — decision 2 specifies the mechanism as "an always-on checklist line", which shipped correctly — hence non-blocking rather than a locked-decision breach | `review-protocol.md` step 0 vs `:328` checklist line | added a "**Record which happened, every time.**" paragraph naming independent/delegated/skipped and why silence is the defect |
| F3 | non-blocking | spec-unclear | fixed inline | Step 0 briefed the delegate adversarially but never said what to hand it, while step 1 carries the load-bearing in-tree rule (read the ticket from the base branch) — a delegate left to find its own context can audit a stale ticket, the exact hazard step 1 exists for | `review-protocol.md` Trigger paragraph vs step 1 | added a clause: hand it the ticket as step 1 reads it, the branch, and the child's configured commands |
| F4 | non-blocking | docs-gap | fixed inline | The manual's cross-reference said step 0 "**now** states…" — release-relative voice that ages in a manual — used a bare relative path where the sibling convention is "the skill's `resources/…`" (`lifecycle.adoc:113-114`), and asserted rule-hood unqualified on a page that disclaims being a rule | `docs/user-manual/concepts/agent-session-workflow.adoc:75` | rewrote: "the skill's `resources/review-protocol.md` step 0 requires the audits to be delegated whenever the reviewing agent wrote the branch it is auditing. The session and tier split below stays a suggestion." |
| F5 | non-blocking | spec-unclear | fixed inline | The degradation clause jumped straight to a conscious skip when no sub-agent was available, skipping the stronger mitigation named in the very next paragraph — a fresh session with no memory of the branch. A harness without sub-agents would record a skip where a session handoff was available | `review-protocol.md` degradation clause | ordered them as fallbacks: fresh session first, conscious skip only when neither is available |
| F6 | non-blocking | design | fixed inline | Path-qualification drift inside the payload: the new text wrote bare "(`resources/tickets-README.md` §8)" where the same file writes "this skill's `resources/…`" at `:7` and `:238` — the phrasing the foreign-workspace convention prefers, since a bare path reads as repo-relative to a foreign reader | `review-protocol.md:71` as shipped | prefixed "this skill's" |
| F7 | non-blocking | design | noted | Decision 8 asked manual and payload to cross-reference rather than restate; the payload's "Session and tier" paragraph does restate the manual's recommendation near-verbatim, so the two can drift | `review-protocol.md` Session-and-tier vs `agent-session-workflow.adoc:73-74` | Unavoidable: the payload cannot point at pickle's own manual, which does not exist in a foreign workspace, and Task 1 mandated the bullet. The manual now points at the payload — a one-way reference is the best available shape. Closed. |
| F8 | non-blocking | docs-gap | noted | `lifecycle.adoc` promises "what a review does" and describes reviews end-to-end without mentioning who runs the audits | `docs/user-manual/concepts/lifecycle.adoc:4`, `:82` | Refinement explicitly scoped the manual change to `agent-session-workflow.adoc`, having verified no other page enumerates the protocol's steps; coverage exists there and in `CHANGELOG.md`, so this is a nice-to-have that does not meet the promotion test. Closed. |

dispositions: 6 fixed inline (F1–F6), 2 noted (F7, F8), 0 folded, 0 promoted — no follow-up ticket
spawned; none passed §5's promotion test.
cost: estimated S, actual S

**Impact sweep (step 8):** no ticket in `1-to-do/` or `2-ready/` lists T-112 in `depends-on:`.
Grepped both directories for references to the review protocol's step numbering and to reviewer
independence — none found, and step 0 is purely additive, so no ready ticket's assumptions moved.
T-113 and T-111 (the next READY items) touch release/scaffold ground and are unaffected.
...
- 2026-08-22 — correction to this ticket's `## Review`: the step-4b checklist line states both in-scope docs-readability suggestions were folded into the inline fixes. Only one was — F4 reworded the manual sentence; the suggestion against this ticket's own new `CHANGELOG.md` entry was never applied. No shipped behaviour is affected (the entry is accurate, only densely worded). The missed suggestion is carried as item 1 of T-114 rather than re-opening this ticket. The Review table is left as written, being the record of what the review actually did.
