...
2026-08-09 — reviewed on `feat/T-087-manual-build-visible-and-testable` (3 commits, rebased onto
`main` @ `9210786` during the review). **Verdict: no blocking findings → DONE.**

- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a)
- [x] Docs-readability pass on `RELEASING.md` / `CHANGELOG.md` (step 4b) — two suggestions on
      this branch's own prose applied (split the long *Fixed* bullet; unpack the dense
      `422 already_exists` warning); the rest landed on pre-existing sections and were discarded
...
| item | result |
|---|---|
| 1 — `bash -n` (+ `shellcheck`) | ✅ both clean |
| 2 — both workflows valid YAML | ✅ `ok` |
| 3 — `just build` / `test` / `lint` / `docs-check` | ✅ all clean |
| 4 — local end-to-end on macOS (`. /tmp/manual-check`) | ✅ 26 s, both files produced, diagnostics + `snowball doctor` → `toolchain ok` |
| 5 — CI `manual-smoke` (run 31318708209) | ✅ success; artifact 662 KB; log shows `MISS Bundler` → `ok Bundler 4.0.16` → `toolchain ok` |
| 6 — `release.yml` path | not testable pre-merge, as the plan states |

Items 1–4 were re-run again after the inline fixes below and after the rebase; all green. All
five tasks landed in the files they name, and all eight confirmed decisions were honoured — D7 in
particular is *proven*, not asserted, since item 4 ran on a developer macOS box.

### Findings

| id | severity | disposition | finding | evidence | suggestion |
|---|---|---|---|---|---|
| F1 | non-blocking | fixed inline | `main` carried this ticket in **both** `3-in-development/` and `4-in-review/` — the move commit never staged the deletion, so a fresh clone of `main` broke the flow's own invariants | `git ls-files tickets/` returned two paths at `1afb340`; `git archive HEAD` piped into `pickle board audit` → 2 errors (duplicate id; BOARD.md rows differ) | fixed on `main` as `9210786`; audit back to 0 errors / 0 warnings |
| F2 | non-blocking | fixed inline | `RELEASING.md` advertised `command -v -a ruby gem bundle` in the diagnostics block, but the script runs `type -a` — and `command -v` has no `-a` in bash, so the documented command does not exist | `RELEASING.md:65` vs `build-manual.sh:72` | corrected the doc to `type -a` |
| F3 | non-blocking | fixed inline | The shim's stated mechanism is contradicted by **this ticket's own CI ground truth** — the very evidence it was filed to collect. Run 31318708209 never printed `bootstrapping via gem install`, yet `bundle --version` → 4.0.16 immediately after: the PATH-prepend half alone sufficed. Ruby's *keg* does ship `bundle`; only the *linked* `$(brew --prefix)/bin` symlink is removed, and the `gem install` fallback has never fired in CI | `build-manual.sh:75-82` and `RELEASING.md:87-89` vs run 31318708209's diagnostics block | rewrote both to state the measured mechanism, cite the run, and flag the fallback as untested |
| F4 | non-blocking | fixed inline | The shim comment claimed it "short-circuits to a no-op" once `bundle` resolves, but the `$(brew --prefix ruby)/bin` prepend ran **unconditionally** — on a developer box that silently promotes Homebrew's ruby ahead of an mise/rbenv-selected one | `build-manual.sh:86-89` (pre-fix); the local item-4 run reported `toolchain ok` *before* the prepend and took it anyway | moved the prepend inside the `if ! command -v bundle` guard, so the comment is now true of the code |
| F5 | non-blocking | fixed inline | `manual-smoke.yml` interpolated `${{ inputs.ref }}` straight into a `run:` body — the standard Actions script-injection hole | `manual-smoke.yml:57` (pre-fix) | pass it through `env: REF:` and test `"$REF"`; behaviour-identical. Also added `permissions: contents: read` |
| F6 | non-blocking | fixed inline | The four `HOMEBREW_*` settings existed in **three** places (script defaults plus both workflows' `env:`), with the tap-trust / broken-pipe rationale duplicated between `release.yml` and the script — the exact "must not exist twice" that Decision 2 extracted the script for | `release.yml:44-50`, `manual-smoke.yml:49-53`, `build-manual.sh:23-31` | deleted both workflow `env:` blocks; the script's `:=` defaults already let a caller override |
| F7 | non-blocking | fixed inline | `CHANGELOG.md` said the build had been failing "since v0.2.1" and `RELEASING.md` said "three releases in a row each hit a different toolchain failure" — but v0.2.1 is the one release that *did* attach its manual | `gh release view v0.2.1 --json assets` lists both manual assets; v0.2.2 and v0.3.0 list none | corrected both to name v0.2.2 and v0.3.0 as the two that shipped without one |
| F8 | non-blocking | new ticket (**T-088**) | No static check for the CI shell/workflow surface: `just lint` is `go vet` + `gofmt` and `ci.yml` has neither `shellcheck` nor `actionlint`. `actionlint` would have caught F5 automatically, and acceptance item 1's "shellcheck if installed" makes the check a property of the laptop, not the repo | `justfile:18-20`, `.github/workflows/ci.yml` | T-088 |
| F9 | non-blocking | new ticket (**T-088**) | `manual-smoke.yml` declares no `concurrency:`, so a burst of `docs/**` pushes queues several ~4-minute Homebrew installs against each other (its missing `permissions:` was fixed inline under F5) | `.github/workflows/manual-smoke.yml` | T-088, batched with F8 as one workflow-hygiene theme |
| F10 | non-blocking | noted | `build-manual.sh` resolves `<out-dir>` relative to `<src-dir>` — the `snowball build -o` runs inside the `cd "$src_dir"` subshell — which the usage comment does not say | `build-manual.sh:12-14` (usage) vs its final subshell | harmless today: both workflows and the acceptance test pass absolute paths. Recorded for whoever passes a relative one |
| F11 | non-blocking | noted | The plan's Finish step 5 (post-merge backfill of the v0.2.2 / v0.3.0 manuals) is genuinely outstanding and cannot be done before the human merges | `gh release view v0.2.2 / v0.3.0 --json assets` → no manual assets on either | carried forward as an explicit pending-action History line below, so it survives the move to `6-done/` |

**Disposition summary:** 11 findings, 0 blocking. 7 fixed inline (F1–F7), 2 folded into one new
ticket by theme (F8, F9 → **T-088**), 2 noted (F10, F11). No finding required rework.

### Notes

- F1 was fixed on `main`, not on the branch: it is overarching bookkeeping, which the rules §0
  keep off a feature branch.
- The inline fixes were folded into the three existing commits as fixups (root-path child → tidy
  into atomic commits), so the branch still reads as three commits. `backup/pre-review-T-087`
  holds the pre-fix tip.
- **Impact sweep (step 8):** no ticket in `1-to-do/` or `2-ready/` lists T-087 in `depends-on:`
  or leans on the release workflow's shape. T-067 (docs link/anchor validation) is adjacent but
  independent — it targets `just docs-check`'s blindness to dead cross-references, not the CI
  surface T-088 covers. No ticket needed patching.
...
- 2026-08-09 — IN DEVELOPMENT → IN REVIEW: acceptance green, PR #25 open
- 2026-08-09 — IN REVIEW → DONE: review: 0 blocking; 7 fixed inline, 2 -> T-088, 2 noted
- 2026-08-09 — **pending action, post-merge** (review finding F11, carried out of `## Review` so
  it survives here): once PR #25 is merged, run `gh workflow run manual-smoke.yml -f ref=v0.3.0`
