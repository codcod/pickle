...
- [x] Reviewer independence settled (step 0): **delegated** — the orchestrating reviewer authored
  this branch in the same session, so a fresh sub-agent ran the audits (steps 2–4a), briefed
  adversarially and pointed explicitly at the payload edits and the foreign-workspace test. Every
  delegated finding was re-verified by hand; the two mutation-testing claims (F1, F2) were
  re-run independently rather than taken on trust, and both reproduced exactly.
- [x] Implementation audit — acceptance test re-run, tasks & criteria verified (step 2): all four
  tasks done in the named files; all four confirmed design decisions honoured, including
  decision 4 (`git diff` shows zero edits to `TestTicketNewAcceptsAwkwardButLegalTitle`).
  `just build`, targeted `-run`, `just test`, `just lint`, `just docs-check` all PASS. The
  amended manual repro re-run in a throwaway dir per the self-modify policy: all three
  terminators and the 250-char title exit 1, write no file, leave the audit clean.
- [x] Quality audit (step 3): rejection still precedes every write (`validateTitle` at the top of
  `runTicketNew`, before `createExclusive` and any board write) — no partial state. Decision 2's
  filename claim independently re-derived and holds: `Slugify` lowercases rune-wise and collapses
  non-`[a-z0-9]` runs, so the slug is pure ASCII and never longer in runes than the title — 120
  runes is therefore also 120 bytes, which is what `NAME_MAX` actually measures. Two real test
  gaps found (F1, F2) and closed.
- [x] Consistency audit (step 4): whole-tree sweep for stale statements of the contract found one
  (F3). 120 now stated consistently in code, tests, manual, payload. The
  `board.maxCellRunes` relationship is correctly described as a shared number, not shared code.
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a). Both payload
  edits judged against `AGENTS.md`'s foreign-workspace test and pass: no ticket id to look up, no
  count from a corpus the reader lacks, no first-person "this repo", no path that resolves only
  in pickle's own tree. `payload_lint_test.go` passes, and its four mechanical rules are
  explicitly *not* what cleared these — the judgement above is, since the test cannot tell
  whether "~120 runes" means anything to a foreign reader.
- [x] Docs-readability pass (step 4b): run on `CHANGELOG.md`, `skill/resources/tickets-README.md`
  and `concepts/tickets.adoc`. Applied the 3 suggestions on prose this ticket authored; discarded
  the rest as pre-existing text this ticket never touched. Polish only; none entered the table.
...
- [x] Remaining-tickets impact sweep done (step 8): T-102 and T-114 patched — see below.
- [x] Summary + child-project commit message & MR attributes presented for approval; overarching
  bookkeeping committed per policy; next-ticket suggestion (step 9)

| id | severity | class | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|---|
| F1 | non-blocking | test-gap | fixed inline | The cap's *accepted* boundary was unpinned — a title of exactly 120 runes had no test, so an off-by-one survived the suite. | Mutating `>` to `>=` in `validateTitle` left `go test ./internal/cli/... -run TestTicketNew` green; re-verified by hand. | Add an exactly-at-the-cap case. Done — new `TestTicketNewAcceptsTitleAtTheCap`, which now fails under that mutation. |
| F2 | non-blocking | test-gap | fixed inline | Decision 2's rune-vs-byte choice was untested: no title near the cap used multi-byte runes, so `utf8.RuneCountInString` could be swapped for `len()` undetected. | Substituting `len(title)` and dropping the import left the suite green; re-verified by hand. | Accept a 120-*rune* multi-byte title (240 bytes). Done — second case in the same new test; it is the one that fails under the `len()` mutation. |
| F3 | non-blocking | docs-gap | fixed inline | A fourth copy of the title contract still described it as "a single line of text" with no mention of the cap — the plan named only three. | `docs/user-manual/concepts/tickets.adoc:19-20` | Add "and reasonably short" plus an xref. Done — same reasoning already applied to `SKILL.md` at pickup, so the tree is now consistent. |
| F4 | non-blocking | other | fixed inline | This branch's manual edit left a 144-char line in a paragraph that wraps at ~78. | `docs/user-manual/cli-reference.adoc:701` | Re-wrap. Done. |
| F5 | non-blocking | spec-unclear | noted | The runtime error still says "title may not contain **newlines**", which reads oddly when what was rejected is an invisible `U+2028`. The manual now defines the term as all five terminators, so code and prose use the word differently. | `internal/cli/ticket.go`'s newline branch vs `cli-reference.adoc:691-700` | Left as-is deliberately: the message predates this branch, "newlines" is the term users actually search for, and changing a user-facing string is an output change — beyond the inline bar and outside this ticket's two findings. |
| F6 | non-blocking | docs-gap | fixed inline | No `## [Unreleased]` CHANGELOG entry for a user-visible behaviour change, though the plan's Docs step omitted it and comparable recent tickets all add one. | `CHANGELOG.md`; `pickle changelog check` listed T-038 as a candidate | Add a `### Fixed` entry covering both findings. Done — `changelog check` no longer names T-038. |
| F7 | non-blocking | docs-gap | noted | **Pre-existing, not this branch:** `pickle changelog check` reports **T-071 and T-111** as shipped-but-unmentioned — both merged in the last two days with no CHANGELOG entry, and neither review caught it. | `pickle changelog check` → "2 candidate(s) shipped but not named in Unreleased" | Not fixed here: this branch did not break it, and the inline bar is about causation, not size. The command is advisory by design ("the entry may legitimately be written any time before the release"), and `RELEASING.md` opens by running it — so a release would catch this. Raised to the user rather than silently absorbed. |

**Disposition summary:** 7 findings, all non-blocking — 5 fixed inline (F1–F4, F6), 2 noted (F5,
F7), 0 folded, 0 new ticket. The two test gaps are the substantive ones: without them the ticket's
central claim (a *rune* cap at *exactly* 120) was asserted by no test that could fail. No blocking
findings.
