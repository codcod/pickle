...
- [x] Reviewer independence settled (step 0): the reviewing agent authored the branch in this
      same session, so audits (steps 2–4a) were **delegated** to an independently spawned
      sub-agent, briefed adversarially with the ticket text, the branch, and the child's
      configured commands. Every delegated finding was re-verified by hand (diff/grep against
      the actual branch) before being recorded below.
- [x] Implementation audit (step 2): all 5 tasks and all 9 confirmed design decisions verified
      against the actual tree on `feat/T-113-scaffold-release-skeletons`, not the prose. The
      acceptance test was re-run verbatim in a throwaway dir with the binary copied in and
      renamed `pickle-test` — every line passed (both files created with correct content;
      `--project-name`/`--force`/`--dry-run` all behave as specified; decision 1's "no `pickle `
      substring" and decision 8's "no other file" checks both held; the `scaffold docs`
      regression check at the end passed, proving the Task 2 extraction was behaviour-preserving).
      `just build`, `just test`, `just lint`, `just docs-check` all green on the branch.
- [x] Quality audit (step 3): idiomatic Go; the new `TestRelease*` cases assert real behaviour
      (content, defaulting, skip/force, dry-run, and two decision-specific regression guards
      `TestReleaseWritesNoOtherFile`/`TestReleaseNamesNoCommand`), not tautologies.
- [x] Consistency audit (step 4): `internal/cli/cli.go` usage() and `internal/cli/scaffold.go`
      error strings updated consistently everywhere "docs" was previously mentioned alone (now
      "docs or release"); no stale "implements `pickle scaffold docs`" text left anywhere.
- [x] Documentation audit (step 4a): `cli-reference.adoc` Overview row + new
      `[#cmd-scaffold-release]` section present and correctly placed; `just docs-check` passes;
      whole-tree sweep found no stale claim elsewhere that `scaffold` has only one subcommand;
      `docs/user-manual/concepts/releasing.adoc` (T-111) still reads correctly, untouched, per
      decision 7. `CHANGELOG.md` `[Unreleased]` entry present for T-113.
- [x] Docs-readability pass (step 4b): ran `docs_readability` on `docs/user-manual/cli-reference.adoc`
      and `CHANGELOG.md`. Every suggestion returned targets pre-existing prose elsewhere in the
      file (install/agent/ticket-new/board sections); none touch this ticket's new Overview row
      or `[#cmd-scaffold-release]` section, and `CHANGELOG.md` was reported as reading well
      as-is — nothing applied, out of this ticket's scope.
- [x] Findings recorded below (step 5); disposition summary + cost line present.
- [x] Ticket moves to `tickets/6-done/` (step 6); `## History` appended.
- [x] No other references needed updating; board regenerated by the move (step 7).
- [x] Impact sweep (step 8): no ticket in `1-to-do/`/`2-ready/` lists T-113 in `depends-on:` or
      Description; none affected.
- [ ] Summary + commit message & MR attributes presented for approval; remote-base check run
      before any push (step 9) — pending, see below.

| id | severity | class | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|---|
| F1 | non-blocking | stale-xref | fixed inline | `Options`' doc comment still said "a single `scaffold docs` run" after this branch made `Options` shared by `Release` too | `internal/scaffold/scaffold.go:55` (pre-fix) | reworded to "configures a single scaffold run (`docs` or `release`)" — fixed inline, commit `6ab143f` |
| F2 | non-blocking | stale-xref | noted | `Options`' doc comment for `ProjectName` says "(Run fills this in)", but no `Run` function exists in this package — pre-existing on `main`, not touched or caused by this branch | `internal/scaffold/scaffold.go:56` | leave as noted; a future editor touching this comment block may as well fix it, but it fails the "did this branch break it" test for inline/blocking treatment here |

Disposition summary: 1 fixed inline (F1), 1 noted (F2). No new tickets, nothing folded.
