...
**Reviewed 2026-08-13** (review protocol; no overarching or per-child addendum configured).
...
- [x] Docs-readability pass on the changed `.adoc`/`.md` files (step 4b) — run; every
      suggestion landed on pre-existing prose outside this ticket's diff, so all were
      discarded rather than applied (widening the diff to unrelated paragraphs is not this
      ticket's job)
...
All four Tasks **met**, all nine confirmed decisions **honoured**:

| Item | Verdict | Evidence |
| --- | --- | --- |
| Task 1 — `idPatterns` + `newIDPatterns` | met | `internal/changelog/changelog.go:88-152`; four shapes rebuilt from one QuoteMeta'd alternation, every original comment preserved and extended; `prTokenRE`/`sectionHeadingRE` left alone (moved out of the var block, unchanged) |
| Task 2 — patterns threaded through | met | `ClassifySubject(subject, prefixes)` wrapper + unexported `classifySubject`; `Check(..., prefixes)` compiles once (`p := newIDPatterns(prefixes)`) and passes `p` to `classifySubject`/`subjectIDs`/`sectionIDs`; package doc rewritten (`changelog.go:15-25`) |
| Task 3 — CLI resolves the prefix set | met | `internal/cli/changelog.go:168-184` `ticketPrefixes`; call site line 99; `printExclusions` doc extended (lines 225-241) |
| Task 4 — tests | met | `TestClassifySubject` gains the `RFC-7231`/`CVE-2024` cases (both now `Neither`), plus `TestNonTicketTokensAreNotIDs`, `TestMultiPrefixRecognition`, `TestEmptyPrefixesFallBackToT`, `TestSectionIDsDoesNotHarvestNonTicketTokens`, and `internal/cli` `TestTicketPrefixes` |
| Decision 1 — one predicate at every site | honoured | no id regex survives outside `idPatterns`; `grep` finds no `idRE`/`boardIDRE`/`trailingIDRE`/`parenIDRE` anywhere (the surviving `internal/ticket.idRE` is an unrelated whole-string validator) |
| Decision 2 — display and `noID` share one predicate | honoured | `printExclusions` still counts `len(ex.IDs) == 0`; no second looser scan exists |
| Decision 3 — T-095's verb-phrase property survives | honoured | `TestCheckBoardCommitWithIDsOutsideLeadingListStillCapturesAll` passes unchanged in substance (only the `prefixes` arg added); this repo's own report names the identical id set on the branch and on `main` (see below) |
| Decision 4 — prefixes from registered children | honoured | `ticketPrefixes(cfg)` = dedup union of `cp.Prefix()` in registration order; no tree walk, no new config key |
| Decision 5 — blind spot accepted + documented | honoured | new `cli-reference.adoc` paragraph (lines 794-802); proved live in the throwaway repo (below) |
| Decision 6 — package stays config-free | honoured | `internal/changelog` imports only `fmt`, `regexp`, `sort`, `strings` |
| Decision 7 — empty slice falls back to `["T"]` | honoured | `newIDPatterns` lines 136-144 + `TestEmptyPrefixesFallBackToT` |
| Decision 8 — `regexp.QuoteMeta` | honoured | `changelog.go:141`, with the belt-and-braces rationale in the doc comment |
| Decision 9 — no exit-code/gating/branch change | honoured | all three `printExclusions` branches intact (`internal/cli/changelog.go:263-274`); `changelog check` exited `0` throughout the acceptance run |

**Commands** (repo root, feature branch): `just build` ✅ · `just test` ✅ (every package `ok`) ·
`just lint` ✅ · `just docs-check` ✅.

**Acceptance test re-run verbatim**, throwaway dir with the built binary copied in (self-modify
policy). All three expected outcomes hold:

```
changelog check: v0.0.1..HEAD, against CHANGELOG.md's "Unreleased" section
  no candidates — every shipped ticket is mentioned
  excluded 2 board: bookkeeping commit(s) mentioning T-001, T-002 (+1 with no ticket id; --show-excluded for subjects)
exit=0
```

1. the exclusion line names `T-001, T-002` and **not** `SHA-256` ✅
2. `(+1 with no ticket id)` fires for the `SHA-256` subject — the restored alarm ✅
3. `RFC-7231` is absent from the candidate list ✅ (before this ticket it printed as a candidate)

**Decision 5 proved live**: registering a second child with `ticket_prefix = "OPS"` made
`feat(ops): do a thing (OPS-1)` surface as a candidate; removing the child made it invisible
again — the documented blind spot, on demand.

**This repo's report unchanged in substance**: branch and `main` both print
`mentioning T-046, T-088, T-091, T-097` (the counts differ by one only because `main` carries
one more `board:` commit than the branch — the in-review move itself).

### Findings

| id | severity | disposition | description | evidence | suggestion |
| --- | --- | --- | --- | --- | --- |
| N1 | non-blocking | note and close | `defaultPrefix = "T"` duplicates `config.DefaultTicketPrefix` with nothing pinning the two together — a future rename of the config constant would silently desync the fallback. The duplication itself is correct (decision 6 forbids the import); only the drift guard is missing. | `internal/changelog/changelog.go:73-76`; no test references both symbols | a one-line assertion in `internal/cli` (which already imports both packages) would pin it; too small to schedule on its own |
| N2 | non-blocking | note and close | `ClassifySubject` now has **zero** non-test callers: `Check` calls the unexported `classifySubject`, so the exported wrapper exists only for the test table. On `main` it had a production caller. | `grep -rn ClassifySubject --include=*.go` — only `changelog_test.go` outside the package's own doc comments | leave it: Task 2 prescribed the split, and the exported form is the package's documented entry point for a single subject |
| N3 | non-blocking | note and close | No test covers the `board` shape *failing* on an unregistered prefix. The other three shapes each have one (`RFC-7231` → `trailing`, `CVE-2024` → `paren`, `SHA-256` → `any`). | verified manually: `ClassifySubject("board: OPS-7 filed", []string{"T"})` = `(Bookkeeping, "")`, and `Check` reports `IDs: []` | add the case to `TestMultiPrefixRecognition`; behaviour is already correct, only unpinned |
| N4 | non-blocking | note and close | The decision-5 docs note lists what an unregistered-prefix id is *not* ("never reported as a shipped candidate, never counted into an exclusion summary") but omits the second-order effect: a `board:` subject whose only id has an unregistered prefix now counts into the `(+N with no ticket id)` clause. Defensible — a renamed prefix genuinely *is* drift — but unstated. | `docs/user-manual/cli-reference.adoc:794-802`; probe in N3 | one clause in that paragraph, next time the section is touched |
| N5 | non-blocking | note and close | The Implementation Plan's acceptance-test recipe is not runnable verbatim: `pickle install` scaffolds no `CHANGELOG.md`, so the first `./pk changelog check` in the throwaway repo exits `1` with `open …/CHANGELOG.md: no such file or directory`. Recorded against the *plan*, not the code — the review supplied the missing file and every assertion then held. | run log above | future changelog-check tickets should seed a `CHANGELOG.md` in the recipe |

**Disposition summary:** 5 findings, 0 blocking · **note and close ×5** (N1–N5) · 0 fixed
inline · 0 absorbed by an existing ticket · **0 new tickets** — none passes the rules §5
promotion test (each is a one-line polish, and batching five one-liners into a ticket nobody
would schedule is exactly the deferral that test exists to prevent).

### Impact sweep (step 8)

No ticket in `1-to-do/` or `2-ready/` lists T-097 in `depends-on:`. The only ticket that
mentions `changelog check` at all is **T-088**, which merely anticipates being reported as a
candidate under a `T-` id — still recognised, so nothing to patch. No sweep edits were needed.
