...
- [x] Docs-readability pass (step 4b) — **conscious skip**: the ticket changed no `.adoc`/`.md`
      file (only `justfile`, three workflow `.yml` files and a `.go` test), so there is no prose
...
All five Tasks **met**, all ten confirmed decisions **honoured**:

| Item | Verdict | Evidence |
| --- | --- | --- |
| Task 1 — `justfile` recipes | met | `justfile`: `lint: lint-ci-surface` + the new `lint-ci-surface` recipe, warn-and-continue when a tool is absent |
| Task 2 — `ci-surface` job | met | `.github/workflows/ci.yml`; ran green on PR #41 (`build-test`, `goreleaser-check`, `ci-surface` all started `11:13:59Z` — decision 4's parallelism confirmed by `gh run view`'s per-job timestamps) |
| Task 3 — three workflows' hygiene | met | `ci.yml` gained `permissions: contents: read` + PR-only-cancelling `concurrency:`; `manual-smoke.yml` gained `concurrency:` keyed on `inputs.ref`/`'self'`; `release.yml` gained `concurrency: cancel-in-progress: false`. `release.yml`'s `contents: write` left untouched, as decided |
| Task 4 — shim shellcheck test | met | `internal/hook/hook_test.go`: `TestShimPassesShellcheck`; verified PASS with shellcheck on PATH and **SKIP** (not pass) with it off PATH |
| Task 5 — fix findings | met (nothing to fix) | Both tools ran clean on the existing surface: `actionlint` (repo root) and `shellcheck .github/scripts/*.sh` both exit 0; the materialised `Shim()` passes `shellcheck -s sh` with no findings. `build-manual.sh` needed no changes |
| Decision 1 (optional locally, hard in CI) | honoured | `PATH=/usr/bin:/bin just lint-ci-surface` → two `warning:` lines, exit 0; a planted `foo=$(ls); echo $foo` script → `SC2086`, exit non-zero |
| Decision 2 (pinned tag, not `@latest`) | honoured, with a documented amendment | Pinned to `v1.7.12`, not the ticket text's `v1.7.7` — checked at implementation time via the GitHub API and `v1.7.7` was five minor releases stale (flagged as a non-blocking finding by the pre-pickup applicability-gate audit, dispositioned inline per decision 2's own "then-current tag" wording). Recorded in a step comment in `ci.yml` |
| Decision 3 (shellcheck presence asserted) | honoured | `shellcheck present` step; PR #41's log shows it printed `version: 0.9.0` |
| Decision 4 (own job, not folded into `build-test`) | honoured | see Task 2 evidence above |
| Decision 5 (actionlint+shellcheck together) | honoured | both installed/asserted in the same `ci-surface` job |
| Decision 6 (three different concurrency settings) | honoured | see Task 3 |
| Decision 7 (Go test, not justfile, for the shim) | honoured | see Task 4 |
| Decision 8 (`-s sh`, not default dialect) | honoured | `hook_test.go`: `exec.Command("shellcheck", "-s", "sh", path)` |
| Decision 9 (fix or narrowly suppress) | honoured (nothing to fix or suppress) | no findings from either tool; no `# shellcheck disable` or `.github/actionlint.yaml` was needed |
| Decision 10 (no zizmor) | honoured | not added |

Acceptance test — both halves green:

- **Local**: `just build && just test && just lint && just docs-check` clean on `main` (post-merge)
  and on the feature branch pre-merge; skip-path and real-finding-fails checks reproduced
  exactly as specified; `TestShimPassesShellcheck` PASS/SKIP both reproduced.
- **CI**: PR #41's checks (`gh pr checks 41`) — `build-test`, `ci-surface`, `goreleaser-check`
  all **pass**; `ci-surface`'s `shellcheck present` step printed a version; all three jobs
  started within the same second, confirming decision 4. The concurrency-cancellation half
  (pushing a second commit to observe a superseded PR run cancelled) was not separately
  re-exercised after the fact — the branch was merged before this review ran — but the
  `cancel-in-progress` expression itself was validated by `actionlint` accepting it and by
  `ci.yml`'s `concurrency:` block being present and correctly scoped; this is noted rather than
  hidden (non-blocking, see findings below).

### Quality audit (step 3)

Idiomatic for both Go and the justfile/YAML surface. `TestShimPassesShellcheck`'s doc comment
states why it exists (T-057 decision 3, the shim being a string literal no file-linter sees) and
mirrors the style of the neighbouring `TestShimBlocksOnlyExitCodeOne`/`TestShimExitCodes`. Error
handling: `shellcheck`'s combined output is surfaced on failure (`t.Errorf("...:\n%s", out)`), and
the `justfile` recipe never swallows a real finding (verified above). No injection risk introduced
(`ci-surface`'s new steps take no untrusted input; `manual-smoke.yml`'s pre-existing `env:`-based
mitigation for `inputs.ref` is unchanged).

### Consistency audit (step 4)

No caller/callee drift: `lint-ci-surface` is additive and `lint`'s existing contract (green on a
bare Go-only checkout) is preserved. No contradiction between the three workflows' differing
concurrency settings — each is commented with its own rationale, matching decision 6's explicit
"do not make them consistent". `hook.Shim()`'s existing doc comment (exit-code contract, T-057
decision 3) is unchanged and the new test does not touch shim logic, only asserts on it — no drift
introduced. No dead references, no duplicated logic.

### Documentation audit (step 4a)

No user-facing surface changed (CLI, skill payload, manual all untouched) — matches the ticket's
own "Docs update" section, which pre-settled both non-updates (no `just lint` prose to update; no
`CHANGELOG.md` entry, CI/repo tooling only). `just docs-check` (snowball) is green. The
discoverability obligation the ticket placed on the `lint-ci-surface` recipe comment itself is
met: the comment states both tools are optional locally, that the recipe warns and continues
without them, and that CI runs them for real.

## Findings

| id | severity | disposition | description | evidence | suggestion |
| --- | --- | --- | --- | --- | --- |
| F1 | non-blocking | note-and-close | The acceptance test's "push a second commit, confirm the superseded PR run is cancelled" sub-step was not independently re-exercised in this review — the branch was already merged by the time review ran, so there was no open PR left to supersede. | PR #41 merged as `64a3794` before this review began; `ci.yml`'s `concurrency:` block is present, correctly scoped (`cancel-in-progress: ${{ github.event_name == 'pull_request' }}`), and accepted by `actionlint` | None — the mechanism is standard GitHub Actions behaviour and the expression is correct; re-verifying it live on a future PR that touches `ci.yml` again is sufficient future coverage |

**Disposition summary**: 1 finding, 1 note-and-close (F1). No follow-up ticket — the gap is a
timing artifact of this review running after merge, not a defect in the implementation.
...
- 2026-08-13 — IN REVIEW → DONE: review passed, 1 non-blocking noted (F1)
- 2026-08-13 — merged to main (PR #41, 64a3794)
