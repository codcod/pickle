...
Reviewed 2026-08-07 on `feat/T-073-introduce-brine-name` (ticket read from the base branch per
the protocol's stale-worktree rule). No review addenda are configured — `pickle.toml` sets
`review_addendum` at neither the overarching nor the child level — so this is the generic
protocol only.
...
**Implementation audit — all 8 tasks met.** `git diff main...feat/T-073-introduce-brine-name`
is 21 files and maps 1:1 onto Tasks 1–8 with no scope creep. Acceptance test re-run verbatim:
`just build`, `just test`, `just lint`, `just docs-check` all clean, plus the throwaway-install
smoke test (`flow show` → `brine`, `flow list` → `brine`, `doctor -v` → exit 0 with an
`ok: flow: brine` line). All five confirmed design decisions honoured: nothing on disk moved
(`SKILL.md` frontmatter `name:` and every literal `.agents/skills/ticket-flow/` path verified
untouched), `"brine"` is the only value `Validate()` accepts, the marker sentinels were not
touched, and the passed-line landed on `doctor` and not `board audit`. Task 4's golden file and
Task 6's hand-mirrored `AGENTS.md` marker block were both verified **byte-exact** against a
fresh `MarkerBlock(cfg)` render.

### Findings

| id | severity | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|
| F1 | non-blocking | noted | No `CHANGELOG.md` entry for the new `pickle flow show\|list` command and `flow` key. **The reviewer's first pass called this blocking and was wrong** — the repo has no such gate. | T-069, the immediately-prior merged ticket, added no entry and passed both a review and a scoped re-review; `T-058`/`T-059`/`T-061`'s entries were written by the human at release-cut time (`48a2c55`, `e98e103`), not by the feature diff; T-057 only carried one because *its own plan* made it a task (`tickets/6-done/T-057-*.md:392`) | Leave to the human at the next release cut, which is this repo's actual practice |
| F2 | non-blocking | fixed inline | Naming-prose sites the plan's enumeration missed: `installation.adoc:57` still read `# lay down the ticket flow` while this branch renamed the identical snippet in `README.md`, and `docs/README.adoc:16` (`how the ticket flow works`) sat 10 lines above a line the branch did rename | `docs/user-manual/installation.adoc:57`, `docs/README.adoc:16` vs `README.md:43`, `docs/README.adoc:26` | Fixed. **`installation.adoc` uses the literal word, not `{flow}`** — the line is inside a `[source,sh]` listing block, where AsciiDoc does not substitute attributes (verified: no attribute is used inside any listing block in this tree, and no `subs=` exists anywhere), so `{flow}` there would have rendered literally and shipped a bug |
| F3 | non-blocking | fixed inline | The self-hosting claim this branch reworded in `AGENTS.md` is repeated verbatim in three hand-written dev-tooling docs that were left saying "ticket flow", contradicting it | `.pi/README.md:3`, `.opencode/README.md:3-4` and `:28`, `opencode.jsonc:2` | Fixed — same wording as `AGENTS.md`'s "the very brine flow it ships" |
| F4 | non-blocking | folded → T-066 | `docs/user-manual/cli-reference.adoc` has no entry for `pickle flow`. Pre-authorised by this ticket's own Docs-update section, which names T-066 as the home | `cli-reference.adoc` has no `[#cmd-flow]`; this ticket's "Docs update" paragraph | T-066 already owns "flags the CLI has and `cli-reference.adoc` never mentions" — add `pickle flow show\|list` to its item 1 |
| F5 | non-blocking | folded → T-074 | The shipped agent scaffolds and three Go package comments still say "ticket flow"/"ticket-flow skill". **Deliberately not fixed inline**: `agents/**` is byte-compared by `doctor`, so editing it would raise a drift warning in every already-installed project until it upgrades — a behaviour consequence, which fails the inline bar | `agents/opencode/opencode.jsonc:3,4,42`, `agents/pi/extensions/*.ts`, `.pi/extensions/docs-readability.ts:49`, `main.go:2`, `assets.go:7`, `internal/install/install.go:1` | T-074 already lists these exact files in its migration scope, where the upgrade path that makes them safe to change also lives |
| F6 | non-blocking | noted | `runFlowShow` and `runFlowList` have byte-identical bodies | `internal/cli/flow.go:29-51` | Left as-is on purpose: the two legitimately diverge the moment a second flow exists (`list` enumerates, `show` prints one), so collapsing them now is churn we would undo in T-080/T-081 |

**Disposition summary:** 6 non-blocking, 0 blocking — 2 fixed inline (F2, F3), 2 folded
(F4 → T-066, F5 → T-074), 2 noted (F1, F6). No follow-up ticket minted: every promotable
finding had an existing owner, which is the batching rule working as intended.

**Docs-readability pass (step 4b).** Run over the 11 changed `.adoc`/`.md` files. It returned
no defects — only optional prose polish, and all but two of its suggestions target paragraphs
this ticket never touched (rewriting them would balloon a rename's diff into an unrelated
copy-edit). The two that do land on changed lines — splitting the long sentence at
`docs/README.adoc:26` and at `the-flow.adoc:6` — are recorded here and **discarded for this
ticket**: both lines now read `... the {flow} skill ...` exactly as the six other renamed sites
do, and re-flowing two of them would make the rename inconsistent rather than clearer. Per the
protocol these are polish, never findings.

**Verdict: PASS** — no blocking findings. The feature is complete, correct, tested and
documented; the four fixes made during review are naming-prose only, with no behaviour change,
and `just build`/`test`/`lint`/`docs-check` are green after them.
