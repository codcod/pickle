...
**Reviewed 2026-08-25** against `feat/T-118-docs-xref-unrouted-spellings` (`b6b437b`, plus the
review's own `237a4ef`).

- [x] Reviewer independence settled (step 0): **delegated**. The reviewing agent authored the
      branch in this session, so the audits (steps 2–4a) went to **two independent sub-agents**
      spawned fresh with no memory of writing it — one for implementation + quality, one for
      consistency + docs — briefed adversarially. Classification, severity, dispositions, the
      move and the approval gate stayed with the orchestrating reviewer. Every delegated finding
      was **re-verified by hand** before entering this table; one delegated severity was
      overridden (see F1).
- [x] Implementation audit — acceptance test re-run verbatim, all 7 steps (step 2)
- [x] Quality audit (step 3)
- [x] Consistency audit (step 4)
- [x] Documentation audit — coverage, whole-tree sweep, docs build clean (step 4a)
- [x] Docs-readability pass (step 4b) — **conscious skip: not applicable**, the branch changes no
      `.adoc`/`.md` file (`git diff --name-only main...HEAD` = `docs_xref_test.go` alone).
...
All seven acceptance steps re-run independently and reproduced: `just test` green; all 8
`TestDocsScannerPatternsMatchWhatTheyClaim` sub-tests pass; both **load-bearing revert checks**
confirmed (restoring `"_"` as the mask filler fails *only*
`TestDocsMaskEscapedXrefsFillerIsInert`; reverting Task 1's pattern fails *only* page E's
assertion); the five-construct scored table reproduces exactly as the plan predicted;
`just build && just test && just lint && just docs-check` all clean; `git diff --name-only
main...HEAD` is exactly `docs_xref_test.go`.

Two checks worth naming because they bound the change rather than confirm it. **Decision 2
(no double-reporting) was verified empirically, not read**: all six patterns run over 36
constructs with pairwise span-intersection across `inter`/`bare`/`link`/`angle` and
`strict`×`angle` — zero overlaps. **The anchor pattern was diffed over the real book**: old vs
new `docsAnchorLineRe` across `docs/` yields 31 anchors and **0 differences** — no inflation
(which would make dead references resolve) and no collapse.

### Findings

| id | severity | class | disposition | description | evidence | suggestion |
|---|---|---|---|---|---|---|
| F1 | non-blocking | stale-xref | fixed inline | File-header paragraph still described anchor matching as "optionally carrying a role … or reftext" — pre-branch behaviour, false once Task 4 admitted chained roles/options and rejected degenerate reftexts | `docs_xref_test.go:17-18` (pre-fix) | Mirror the pattern's own doc comment; done in `237a4ef` |
| F2 | non-blocking | stale-xref | fixed inline | Header preamble credited the hardening to T-115 alone, though its first bullet now carries T-118 content | `docs_xref_test.go:29` (pre-fix) | Re-attribute to "T-115 and T-118"; done in `237a4ef` |
| F3 | non-blocking | docs-gap | fixed inline | `docsAnchorLineRe`'s comment listed the degenerate form `[#id,]` twice; the second should read `[#id, ]` (blank reftext), which is what the fixture actually pins | `docs_xref_test.go:115` (pre-fix) | One-character correction; done in `237a4ef` |
| F4 | non-blocking | design | noted | The "two edges" preamble introduced three bullets — a miscount **pre-existing on `main`** (verified: `git show main:docs_xref_test.go` has the same 3 bullets under "two") | `git show main:docs_xref_test.go:29` | Not this branch's defect (rules §5, causation not authorship). Subsumed without a separate fix: F2's rewrite drops the count entirely |
| F5 | non-blocking | design | fixed inline | The new comment argued the `.`-exclusion purely as duplicate-avoidance, without recording what it costs: a *dotted extensionless* name (`xref:my.page#anchor[x]`) matches no pattern and stays silent | verified: `inter=false bare=false angle=false site=false`; also silent pre-branch | Record the residual in the comment that argues the exclusion is safe; done in `237a4ef` |
| F6 | non-blocking | test-gap | fixed inline | Decision 5 half-met for Task 4: the fixture rows bounded the *tightening* (reftext) but nothing bounded the *widening* — a future `+`→`*` slip would make `[#id.]` a legal anchor with the whole table still green | `docs_xref_test.go:642-650` (pre-fix) | Add `[#id.]`, `[#id%]`, `[#id..role]` to `noMatch`; done in `237a4ef` and confirmed load-bearing (the slip now fails all three rows) |
| F7 | non-blocking | design | noted | `<<#anchor>>` and `<<a#>>` (empty name / empty anchor halves) still score `site=true` with every detector false — the coverage-invariant misreport item 2 exists to remove, surviving for these two degenerate shapes | verified: `angle=false strict=false site=true` | **Pre-existing** (identical before the branch) and fails loudly, never silently. Neither shape appears in the manual. Closed with its evidence |
| F8 | non-blocking | design | fixed inline | `TestDocsMaskEscapedXrefsFillerIsInert` sliced the masked line via `strings.Index(got, "and")` — sound today (a missing substring panics rather than passing vacuously) but it states the intent indirectly | `docs_xref_test.go:927` (pre-fix) | Slice by the masked construct's own length via a named constant; done in `237a4ef` |

**Disposition summary:** 8 non-blocking, 0 blocking — 6 fixed inline (F1, F2, F3, F5, F6, F8),
2 noted (F4, F7); 0 folded, 0 new tickets.

`cost: estimated S, actual S`

### Notes on two judgement calls

**F1's severity was overridden.** The delegated consistency reviewer proposed it as *blocking*.
Downgraded to non-blocking after checking it against the rules' own test: a false comment in a
test file ships no wrong behaviour, breaks no golden path, and contradicts no *confirmed design
decision* (the plan's Task 2 named the inter-document bullet, not this paragraph). What it **is**
is prose *this branch made false* with no behaviour change — which is precisely the case rules §5
carves out for `fixed inline`, on condition it is recorded rather than silently edited. It is
recorded above.

**Why F4 and F7 were not fixed despite being one-liners.** Both are pre-existing, and rules §5 is
explicit that the inline bar is about causation, not size: "did this branch break it?", not "is it
small?". F4 additionally needed no fix of its own — F2's rewrite removes the miscounted clause
altogether. Neither passes the promotion test ("would this actually be scheduled?"): F7 fails
loudly on a shape the manual does not contain, so it is closed with its evidence for a later
reviewer to promote by citing this row.
