...
Reviewed 2026-08-13 on `feat/T-085-aggregable-ticket-record`. Round 1 (HEAD `cc6fc43`, 5
commits): **REWORK** — 2 blocking, 5 non-blocking. Scoped re-review after round 1 (HEAD
`83d6485`, 7 commits): **DONE** — all four in-scope findings verified fixed, no regressions.

> **Independence caveat.** The scoped re-review was run by the same agent that performed the
> rework, so it lacks the reviewer independence `agent-session-workflow.adoc` recommends for
> severity calls. It was therefore run against the tree — re-reading each changed hunk, re-running
> every acceptance check, and re-installing the payload into a throwaway workspace — rather than
> against memory of what the rework intended. Round 1's findings were raised by a fresh read.
...
**Disposition summary:** 7 findings — 2 blocking (F1, F2 → rework round 1, both fixed and
verified, no disposition); 5 non-blocking, all dispositioned: 2 *fixed inline* (N1, N5 — taken
in the same rework pass), 2 *new ticket* (N2, N3 — batched as one theme into **T-098**, "the
shipped payload cites this repo's own ticket ids and corpus as if the reader could look them
up", filed `--spawned-by "T-085"`), 1 *noted* (N4). Spawn count 1 ticket / 5 non-blocking
findings = **R 0.2**.

```
cost: estimated S, actual S — four payload edits plus one rework round of two blocking findings,
all prose, inside one session; no Go change at any point. The rework did not push it to M: F1 and
F2 were ~30 lines across three files. Per N4, this line is written on the concluding verdict and
covers the rework round.
```
...
is already applied on `main` in `efb6b97`.) **Applied at the concluding review**: T-065's open
scope question now names the canonical skeleton and the seven-column list, notes that
`TEMPLATE.md` points rather than states, and carries its own dated impact-sweep History line.
Nothing re-graded — the 2026-08-04 precedent refusing prospective demand still holds, and this
ticket's standing instruction says so explicitly.
...
### Scoped re-review (2026-08-13) — verdict DONE

Only the four findings fixed in round 1 were re-verified, per the protocol's scoped-re-review
rule; the feature was not re-audited from scratch.

| finding | verified | how |
|---|---|---|
| **F1** | **fixed** | Read both hunks in the tree. `tickets.adoc:94-101` now enumerates four History line kinds and scopes the length limit to transition/merge lines — which matches `internal/audit/audit.go:209`, so the manual and the implementation now agree. `lifecycle.adoc:107-122` adds the `class` axis and the two closing summary lines, and it **points** at `review-protocol.md` §5 instead of restating the eight values, so decision 4's single-source-of-truth rule survives into the manual. Its "the skill's `resources/review-protocol.md`" form matches the convention `lifecycle.adoc:5` already sets. |
| **F2** | **fixed** | `SKILL.md:157-165` names the placeholder, instructs the overwrite, gives the five-value form inline and defers to rules §1 for definitions. Verified **as installed**, not just in the repo: a fresh `pk install` into `$(mktemp -d)` puts the new step 5 into `.agents/skills/ticket-flow/SKILL.md`, so the fix reaches a real workspace through the embedded payload. |
| **N1** | **fixed** | `TEMPLATE.md:143` now reads `review-protocol.md` §5, matching line 141. |
| **N5** | **fixed** | `SKILL.md:116-123` and `:255-260` both name `class` with a pointer and no restatement. |

Regression checks: `just build`, `just test`, `just lint`, `just docs-check` green; the
no-Go-code guard prints nothing (branch touches 7 files, all `.md`/`.adoc`); acceptance checks
1–5 all hold — in particular check 1 still finds exactly **one** statement of the column list and
check 2 still finds `spec-unclear` only in `review-protocol.md`, confirming the added SKILL.md
and `.adoc` prose pointed rather than duplicated. E2E in a fresh throwaway workspace: `board
audit` 0 errors.

One defect was found *in the rework itself* and fixed before concluding: the F2/N5 edits left two
lines over the file's ~100-column wrap (`SKILL.md` had exactly one such line on `main`, so the
convention was unambiguous). Rewrapped and the commit amended — `83d6485`.
