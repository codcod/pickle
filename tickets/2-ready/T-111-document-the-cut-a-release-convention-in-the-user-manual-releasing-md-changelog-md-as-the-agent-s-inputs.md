...
just test
```
All clean. **T-067 (docs xref/anchor validation) is not yet done**, so `just docs-check` will not
mechanically catch a dangling `<<releasing>>`/`<<cmd-changelog-check>>`/`<<agent-session-workflow>>`
target — verify each of the three new/added cross-references by hand (grep the target anchor
exists: `grep -rn '\[#releasing\]' docs/`, etc.) before presenting, and re-run this check if T-067
lands first and this ticket rebases onto it. Also render the manual locally once
