...
| 9 | **`pickle install --hooks` is untested** (added 2026-08-06 by the T-057 review, finding N7, disposition `folded`). `runInstall`'s hook block (`internal/cli/install.go:97-107`) has no automated coverage at all: neither the success path nor its deliberate "a hook failure is a *warning*, the install still succeeds" branch appears in `internal/cli/*_test.go`. It is exercised only by T-057's 12-step acceptance transcript, which neither `just test` nor CI runs. Same shape as items 1 and 4 — a cli-level test driving `runInstall` against a temp root, one case with a real `git init` (hook installed, path printed) and one where `hook.Install` must fail (a foreign `pre-commit` in place) asserting exit 0 plus the warning on stderr. |
